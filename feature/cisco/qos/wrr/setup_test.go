...
var (
	...
	baseConfigEgressFile     = "base_config_arista_scheduler.json"
	...
	baseConfigQm             = "queue-management1.json"
)
...
func BaseConfigQm() *oc.Qos {
	sl := strings.Split(setup.FindTestDataPath(), "/")
	sl = sl[:len(sl)-1]
	baseConfigPath := strings.Join(sl, "/") + "/" + baseConfigQm
	jsonConfig, err := os.ReadFile(baseConfigPath)
	if err != nil {
		panic(fmt.Sprintf("Cannot load base config: %v", err))
	}
...
func setupQosQm(t *testing.T, dut *ondatra.DUTDevice) *oc.Qos {
	bc := BaseConfigQm()
	setup.ResetStruct(bc, []string{"QueueManagementProfile"})
	bcQm := setup.GetAnyValue(bc.QueueManagementProfile)
	dut.Config().Qos().QueueManagementProfile(*bcQm.Name).Update(t, bcQm)
	return bc
