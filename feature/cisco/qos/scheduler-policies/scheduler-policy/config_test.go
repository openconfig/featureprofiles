	...
	for _, input := range testNameInput {
		t.Run(fmt.Sprintf("Testing /qos/scheduler-policies/scheduler-policy/config/name using value %v", input), func(t *testing.T) {
			...
			*baseConfigSchedulerPolicy.Name = input
			...
			state := dut.Telemetry().Qos().SchedulerPolicy(*baseConfigSchedulerPolicy.Name)

			t.Run("Replace container", func(t *testing.T) {
				config.Replace(t, baseConfigSchedulerPolicy)
			})
			if !setup.SkipGet() {
				t.Run("Get container", func(t *testing.T) {
					configGot := config.Get(t)
					if *configGot.Name != input {
						t.Errorf("Config /qos/scheduler-policies/scheduler-policy/config/name: got %v, want %v", configGot, input)
					}
				})
			}
			if !setup.SkipSubscribe() {
				t.Run("Subscribe container", func(t *testing.T) {
					...
					if *stateGot.Name != input {
						t.Errorf("State /qos/scheduler-policies/scheduler-policy/config/name: got %v, want %v", stateGot, input)
					}
				})
			}
		})
