...
func TestType(t *testing.T) {
	...
	for _, input := range inputs {
		t.Run(fmt.Sprintf("Testing /acl/interfaces/interface/ingress-acl-sets/ingress-acl-set/config/type using value %v", input), func(t *testing.T) {
			...
			config := gnmi.OC().Acl().Interface(*baseConfigInterface.Id).IngressAclSet(*baseConfigInterfaceIngressAclSet.SetName, baseConfigInterfaceIngressAclSet.Type)
			state := gnmi.OC().Acl().Interface(*baseConfigInterface.Id).IngressAclSet(*baseConfigInterfaceIngressAclSet.SetName, baseConfigInterfaceIngressAclSet.Type)

			t.Run("Replace", func(t *testing.T) {
				gnmi.Replace(t, dut, config.Config(), baseConfigInterfaceIngressAclSet)
			})
			if !setup.SkipGet() {
