...
func addPrototoAte(t *testing.T, top *ondatra.ATETopology) {
	...
	if totalisisPfx > gribiscale {
		addAteISISL2(t, top, "atePort8", "B4", "isis_network", 20, innerdstPfxMin_isis+"/"+v4mask, totalisisPfx)
	} else {
		addAteISISL2(t, top, "atePort8", "B4", "isis_network", 20, innerdstPfxMin_isis+"/"+v4mask, gribiscale)
	}

	if totalbgpPfx > gribiscale {
		addAteEBGPPeer(t, top, "atePort8", dutPort8.IPv4, 64001, "bgp_recursive", atePort8.IPv4, innerdstPfxMin_bgp+"/"+v4mask, totalbgpPfx, true)
	} else {
		addAteEBGPPeer(t, top, "atePort8", dutPort8.IPv4, 64001, "bgp_recursive", atePort8.IPv4, innerdstPfxMin_bgp+"/"+v4mask, gribiscale, true)
	}
...
func (a *testArgs) createFlow(name string, srcEndPoint *ondatra.Interface, dstEndPoint []ondatra.Endpoint, innerdstPfxMin string, innerdstPfxCount uint32, opts ...*TGNoptions) *ondatra.Flow {
	...
	if innerdstPfxCount > gribiscale {
		innerIpv4Header.DstAddressRange().WithMin(innerdstPfxMin).WithCount(innerdstPfxCount).WithStep("0.0.0.1")
	} else {
		innerIpv4Header.DstAddressRange().WithMin(innerdstPfxMin).WithCount(gribiscale).WithStep("0.0.0.1")
	}
