...
type eventSendToNonReachableIP struct {
    targetIP   string
    targetPort string
}

type eventSendToShutdownInterface struct {
    destIP string
    port   *ondatra.Port
}

type eventSendToShutInterface struct {
    destIP string
    port   *ondatra.Port
}

func (eventArgs eventSendToNonReachableIP) IsEventType() {}

func (eventArgs eventSendToNonReachableIP) sendPacketsToNonReachableIP(t *testing.T) {
    t.Helper()
    ate := ondatra.ATE(t, "ate")
    top := ate.Topology().New()

    // Define source and destination interfaces
    ateSource := ate.Ports()[0]
    srcIntf := top.AddInterface("src").WithPort(ateSource)
    srcIntf.IPv4().
        WithAddress("192.0.2.1/30").
        WithDefaultGateway("192.0.2.2")

    dstIntf := top.AddInterface("dst").WithPort(ate.Ports()[1])
    dstIntf.IPv4().
        WithAddress(eventArgs.targetIP + "/32").
        WithDefaultGateway("192.0.2.1")

    top.Push(t).StartProtocols(t)

    flow := ate.Traffic().NewFlow("TestFlow").
        WithSrcEndpoints(srcIntf).
        WithDstEndpoints(dstIntf).
        WithHeaders(
            ondatra.NewEthernetHeader(),
            ondatra.NewIPv4Header().WithDstAddress(eventArgs.targetIP),
        )

    // Start the traffic
    ate.Traffic().Start(t, flow)
    t.Logf("Started traffic to %s", eventArgs.targetIP)

    // Wait for some time to allow traffic to be sent
    time.Sleep(30 * time.Second)

    // Stop the traffic
    ate.Traffic().Stop(t)
    t.Logf("Stopped traffic to %s", eventArgs.targetIP)

    //Retrieve and log traffic statistics
    outPkts := gnmi.Get(t, ate, gnmi.OC().Flow(flow.Name()).Counters().OutPkts().State())
    inPkts := gnmi.Get(t, ate, gnmi.OC().Flow(flow.Name()).Counters().InPkts().State())
    t.Logf("OutPkts = %d, InPkts = %d", outPkts, inPkts)
}

func (eventArgs eventSendToShutdownInterface) IsEventType() {}

func (eventArgs eventSendToShutdownInterface) sendToShutdownInterface(t *testing.T) {
    ...
    if errMsg := testt.CaptureFatal(t, func(t testing.TB) {
        gnmi.Replace(t, dut, gnmi.OC().Interface(eventArgs.port.Name()).Enabled().Config(), false)
    }); errMsg != nil {
        gnmi.Replace(t, dut, gnmi.OC().Interface(eventArgs.port.Name()).Enabled().Config(), false)
    }

    //Send packets to the non-reachable IP address
    args := &testArgs{
        ate: ondatra.ATE(t, "ate"),
        top: configureATE(t, ondatra.ATE(t, "ate")),
    }
    flow := args.createFlow("non_reachable_ip_flow", []ondatra.Endpoint{args.top.Interfaces()["ateDst"]}, &tgnOptions{ipv4: true})
    args.ate.Traffic().Start(t, flow)
    time.Sleep(10 * time.Second) // Allow traffic to flow for a short duration
    args.ate.Traffic().Stop(t)
}

// type eventSendNonReachableIP struct{}

// func (eventArgs eventSendNonReachableIP) IsEventType() {}

// func (eventArgs eventSendNonReachableIP) sendPacketsNonReachableIP(t *testing.T, args *testArgs) *ondatra.Flow {
//  t.Helper()
//  args.ate.Traffic().Stop(t)
//  time.Sleep(30 * time.Millisecond)
//  return args.createFlow("invalid_flow_to_non_rweachable_ip", []ondatra.Endpoint{args.top.Interfaces()["ateDst"]}, &tgnOptions{ipv4: true, nonReachableIP: true})
// }

// func (eventArgs eventSendToShutInterface) IsEventType() {}

// func (eventArgs eventSendToShutInterface) sendToShutInterface(t *testing.T) {
//  t.Helper()
//     // Shut down the interface
//     dut := ondatra.DUT(t, "dut")
//  args := &testArgs{
...
//  }
//     gnmi.Replace(t, dut, gnmi.OC().Interface(eventArgs.port.Name()).Enabled().Config(), false)
//     time.Sleep(5 * time.Second) // wait for the interface to shut down

//     // Create traffic stream with the destination IPv6 address of the shut interface
//     srcEndPoint := args.top.Interfaces()[ateSrc.Name]
//     dstEndPoint := args.top.Interfaces()[ateDst.Name]
//  ipv6Header := ondatra.NewIPv6Header().
//         WithSrcAddress(dutSrc.IPv6).
//         WithDstAddress(eventArgs.destIP)
//      flow := args.ate.Traffic().NewFlow("FlowToShutInterface").
...
//     args.ate.Traffic().Start(t, flow)
//     time.Sleep(30 * time.Second) // run traffic for 30 seconds
//     args.ate.Traffic().Stop(t)

//      // Bring the interface back up
//      gnmi.Replace(t, dut, gnmi.OC().Interface(eventArgs.port.Name()).Enabled().Config(), true)
//      time.Sleep(5 * time.Second) // wait for the interface to come up
//  }
...
// Create Stream from TGEN with Destination IPv6 Address of a Shut Interface
...
type tgnOptions struct {
    drop, mpls, ipv4, udp, ttl, nonReachableIP bool
    trafficTimer                               int
    fps                                        uint64
    fpercent                                   float64
    frameSize                                  uint32
...
func (args *testArgs) createFlow(name string, dstEndPoint []ondatra.Endpoint, opts ...*tgnOptions) *ondatra.Flow {
    srcEndPoint := args.top.Interfaces()[ateSrc.Name]
    srcEndPoint1 := args.top.Interfaces()[ateSrc.Name+"1"]
    ...
    for _, opt := range opts {
        ...
        if opt.nonReachableIP {
            hdrIpv4 := ondatra.NewIPv4Header().WithDstAddress("203.1.0.1")
            header = []ondatra.Header{ondatra.NewEthernetHeader(), hdrIpv4}
        }
    }
    flow = args.ate.Traffic().NewFlow(name).
        WithSrcEndpoints(srcEndPoint, srcEndPoint1).
        WithDstEndpoints(dstEndPoint...).
        WithHeaders(header...)
...
func (args *testArgs) validateTrafficFlows(t *testing.T, flow *ondatra.Flow, opts ...*tgnOptions) uint64 {
    ...
    for _, op := range opts {
        if eventAction, ok := op.event.(*eventInterfaceConfig); ok {
            ...
            args.ate.Traffic().Start(t, flow)
        } else if eventAction, ok := op.event.(*eventSendToNonReachableIP); ok {
            eventAction.sendPacketsToNonReachableIP(t)
        }
    }
