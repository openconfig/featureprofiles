...
func testTrafficFlows(t *testing.T, args *testArgs, expectPass bool, flow ...*ondatra.Flow) {
	...
	// log stats
	t.Log("All flow LossPct: ", args.ate.Telemetry().FlowAny().LossPct().Get(t))
	t.Log("FlowAny InPkts  : ", args.ate.Telemetry().FlowAny().Counters().InPkts().Get(t))
	t.Log("FlowAny OutPkts : ", args.ate.Telemetry().FlowAny().Counters().OutPkts().Get(t))

	flowPath := args.ate.Telemetry().FlowAny()
...
// getAllFlows returns a map of all Flows. Various tests use a subset of these flows.
func getAllFlows(args *testArgs) map[string]*ondatra.Flow {
	...
	var f = map[string]*ondatra.Flow{
		"ipinipd10": getIPinIPFlow(args, dstEndPointVlan10, "ipinipd10", 10),
		"ipinipd20": getIPinIPFlow(args, dstEndPointVlan20, "ipinipd20", 20),
		"ipinipd30": getIPinIPFlow(args, dstEndPointVlan30, "ipinipd30", 30),

		"ipinipd11": getIPinIPFlow(args, dstEndPointVlan10, "ipinipd11", 11),
		"ipinipd12": getIPinIPFlow(args, dstEndPointVlan10, "ipinipd12", 12),

		"ipinipd21": getIPinIPFlow(args, dstEndPointVlan20, "ipinipd21", 21),
		"ipinipd22": getIPinIPFlow(args, dstEndPointVlan20, "ipinipd22", 22),

		"ipinipd31": getIPinIPFlow(args, dstEndPointVlan30, "ipinipd31", 31),
		"ipinipd32": getIPinIPFlow(args, dstEndPointVlan30, "ipinipd32", 32),

		"ipinipd10v20": getIPinIPFlow(args, dstEndPointVlan20, "ipinipd10v20", 10),
		"ipinipd11v20": getIPinIPFlow(args, dstEndPointVlan20, "ipinipd11v20", 11),
		"ipinipd12v20": getIPinIPFlow(args, dstEndPointVlan20, "ipinipd12v20", 12),
	}

	return f
}

// getFlows returns a slice of flows based on an input slice of flownames
func getFlows(f map[string]*ondatra.Flow, keys []string) []*ondatra.Flow {
	var flows []*ondatra.Flow
	for _, key := range keys {
		flows = append(flows, f[key])
	}
	return flows
}
...
func TestPBR(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			passingFlows: getFlows(allFlows, []string{"ipinipd10", "ipinipd20", "ipinipd30"}),
		},
		{
			name: "RT3.2 Case2",
			...
			// use IPinIP flows with DSCP11-12 for VLAN10, DSCP21-22 for VLAN20, DSCP31-32 for VLAN30.
			// use IPinIP flows for DSCP10, DSCP20 and DSCP30.
			passingFlows: getFlows(allFlows, []string{"ipinipd10", "ipinipd11", "ipinipd12",
				"ipinipd20", "ipinipd21", "ipinipd22", "ipinipd30", "ipinipd31", "ipinipd32"}),
		},
		{
			name: "RT3.2 Case3",
			...
			// use IPinIP DSCP10-12 flows for VLAN20. Reuse DSCP10-12 flows for VLAN10.
			passingFlows: getFlows(allFlows, []string{"ipinipd10", "ipinipd11", "ipinipd12"}),
			failingFlows: getFlows(allFlows, []string{"ipinipd10v20", "ipinipd11v20", "ipinipd12v20"}),
		},
		{
			name: "RT3.2 Case4",
			...
			policy: getPBRPolicyForwarding(args,
				...
				getL3PBRRule(args, "VRF20", 2, []uint8{20})),
			// use ipinipd10, ipinipd11, ipinipd12 flows to match IPinIP to VRF10
			// use ipinipd20 flow to match IPinIP with DSCP20 to VRF20
			// use ipinipd10v20, ipinipd11v20, ipinipd12v20 flows to show they fail for VRF20
			passingFlows: getFlows(allFlows, []string{"ipinipd10", "ipinipd11", "ipinipd12"}),
			failingFlows: getFlows(allFlows, []string{"ipinipd10v20", "ipinipd11v20", "ipinipd12v20", "ipinipd20"}),
		},
	}
