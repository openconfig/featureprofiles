...
const (
	...
	greTTL             = 64
	greProtocol        = 47
	IPv4_DST_NET       = "192.168.10.1"
	IPv6_DST_NET       = "2001:DB8::10:1"
	IPv4_DST_NET_SERV1 = "192.168.10.11"
	IPv6_DST_NET_SERV1 = "2001:DB8::10:10"
	GRE_IPv4_DST_NET   = "192.168.10.21"
...
var (
	...
	staticRoutes            = map[string]string{
		fmt.Sprintf("%s/32", IPv4_DST_NET):        otgPort2.IPv4,
		fmt.Sprintf("%s/128", IPv6_DST_NET):       otgPort2.IPv6,
		fmt.Sprintf("%s/32", IPv4_DST_NET_SERV1):  otgPort2.IPv4,
		fmt.Sprintf("%s/128", IPv6_DST_NET_SERV1): otgPort2.IPv6,
		fmt.Sprintf("%s/32", GRE_IPv4_DST_NET):    otgPort2.IPv4,
	}
)
...
func TestIngressHandlingOfTTL(t *testing.T) {
	...
	tests := []testCase{
		...
		},
		{
			...
			flowName:    "PF-1.8.5",
			destination: IPv4_DST_NET_SERV1,
			ipType:      IPv4,
			greEncap:    true,
			ttl:         10,
			expectDrop:  false,
		},
		{
			...
			flowName:    "PF-1.8.6",
			destination: IPv6_DST_NET_SERV1,
			ipType:      IPv6,
			greEncap:    true,
			ttl:         10,
			expectDrop:  false,
		...
		{
			name:        "IPv4 traffic with GRE encapsulation on DUT and TTL = 1 with DUT configured NOT to process TTL = 1 on receiving interface",
			flowName:    "PF-1.8.9",
			destination: IPv4_DST_NET_SERV1,
			ipType:      IPv4,
			greEncap:    true,
		...
		{
			name:        "PF-1.8.10: IPv6 traffic with GRE encapsulation on DUT and TTL = 1 with DUT configured NOT to process TTL = 1 on receiving interface",
			flowName:    "PF-1.8.10",
			destination: IPv6_DST_NET_SERV1,
			ipType:      IPv6,
			greEncap:    true,
...
func verifyIcmpMessage(t *testing.T, captureFilename string, tc testCase) {
	...
	// Use gopacket to parse the pcap file
	handle, err := pcap.OpenOffline(captureFilename)
	if err != nil {
		t.Errorf("Failed to open pcap file: %v", err)
		return
...
func configurePolicyForwarding(t *testing.T, dut *ondatra.DUTDevice) {
	...
	if deviations.NextHopEncapHeaderOcUnsupported(dut) || deviations.PolicyForwardingToNextHopOcUnsupported(dut) {
...
func configureNoTtlCheck(t *testing.T, dut *ondatra.DUTDevice) {
	// TODO - configuring the DUT to not check TTL is unsupported on current dut
}
...
func trafficPolicyCliConfig(dut *ondatra.DUTDevice, policyName string, interfaceName string) string {
	...
	case ondatra.ARISTA:
        ...
        !
        `, rulev4, IPv4_DST_NET_SERV1, nhGroupName)
		...
		redirect next-hop group %s
        !
        `, rulev6, IPv6_DST_NET_SERV1, nhGroupName)

		greNH := fmt.Sprintf(`
		nexthop-group %s type gre
		ttl %d
		tunnel-source %s
		entry 0 tunnel-destination %s
		!
		`, nhGroupName, greTTL, otgPort1.IPv4, GRE_IPv4_DST_NET)
		...
		trafficPolicyConfig := fmt.Sprintf(`
            ...
            `, policyName, v4MatchRules, v6MatchRules, greNH, interfaceName, trafficPolicyName)
...
func configurePolicyForwardingFromOC(t *testing.T, dut *ondatra.DUTDevice, policyName string, interfaceName string) {
	...
	// Policy Forwarding Policy and Rule
	policy := pf.GetOrCreatePolicy(policyName)

	rule := policy.GetOrCreateRule(1)
	ruleIpv4 := rule.GetOrCreateIpv4()
	ruleIpv4.DestinationAddress = ygot.String(IPv4_DST_NET_SERV1)

	ruleIpv6 := rule.GetOrCreateIpv6()
	ruleIpv6.DestinationAddress = ygot.String(IPv6_DST_NET_SERV1)

	ruleAction := rule.GetOrCreateAction()
	//ruleAction.Count = ygot.Bool(true)
	ruleAction.NextHop = ygot.String(policyName + "_nh")

	// Next-Hop Group with GRE Encap

	nhg := ni.GetOrCreateAfts().GetOrCreateNextHopGroup(1)
	nhg.NextHopGroupName = ygot.String(policyName + "_nh")

	nhg.GetOrCreateNextHop(0)
	nh := ni.GetOrCreateAfts().GetOrCreateNextHop(1)

	t.Log("Configuring encap header")
	encap := nh.GetOrCreateEncapHeader(uint8(1))
	gre := encap.GetOrCreateGre()

	gre.SrcIp = ygot.String(otgPort1.IPv4)
	gre.DstIp = ygot.String(GRE_IPv4_DST_NET)

	pfEntry := ni.GetOrCreateAfts().GetOrCreatePolicyForwardingEntry(1)
	pfEntry.NextHopGroup = ygot.Uint64(1)
