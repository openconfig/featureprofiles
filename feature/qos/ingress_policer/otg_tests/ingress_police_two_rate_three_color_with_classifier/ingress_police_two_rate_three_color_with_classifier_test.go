...
func configureDUT(t *testing.T, dut *ondatra.DUTDevice) {
	...
	cfgplugins.NewTwoRateThreeColorScheduler(t, dut, qos, schedulerName, inputPolicerName, targetClass, cirValue, pirValue, burstSize, sequenceNumber, targetQueueID, targetQueue)
	cfgplugins.ApplyQosPolicyOnInterface(t, dut, qos, inputInterfaceName, schedulerName)
	pushQosConfig(t, dut, qos)
}
...
func pushQosConfig(t *testing.T, dut *ondatra.DUTDevice, qos *oc.Qos) {
	t.Helper()
	qosPath := gnmi.OC().Qos().Config()
	gnmi.Replace(t, dut, qosPath, qos)
}
...
func runTest(t *testing.T, dut *ondatra.DUTDevice, ate *ondatra.ATEDevice, config gosnappi.Config, tc testCase) {
	...
	otg := ate.OTG()
	flowName := strings.Replace(tc.name, " ", "_", -1)
...
func validateSchedulerCounters(t *testing.T, dut *ondatra.DUTDevice, intf string, tc testCase, droppedPackets uint64) {
	if deviations.PolicyRuleCountersOCUnsupported(dut) {
		...
		t.Errorf("Scheduler counters are not supported in OC")
		return
	}
	scheduler := gnmi.Get(t, dut, gnmi.OC().Qos().Interface(intf).Input().SchedulerPolicy().Scheduler(sequenceNumber).State())
	t.Logf("Scheduler counters for %s at %.1fGbps: conforming=%d, exceeding=%d, violating=%d",
		intf, float64(tc.flowRate/1000), *scheduler.ConformingPkts, *scheduler.ExceedingPkts, *scheduler.ViolatingPkts)
	if *scheduler.ViolatingPkts != droppedPackets {
		t.Errorf("Expected %d dropped packets, but got %d",
			droppedPackets, *scheduler.ViolatingPkts)
	}
}

func validateClassifierCounters(t *testing.T, dut *ondatra.DUTDevice, intf string, tc testCase, expectedPackets uint64) {
	if deviations.PolicyRuleCountersOCUnsupported(dut) {
		t.Errorf("Classifier counters are not supported in OC")
		return
	}
	classifier := gnmi.Get(t, dut, gnmi.OC().Qos().Interface(intf).Input().Classifier(inputClassifierType).Term(targetClass).State())
	t.Logf("Classifier counters for %s at %.1fGbps: matched-packets=%d",
		intf, float64(tc.flowRate/1000), *classifier.MatchedPackets)
	if *classifier.MatchedPackets != expectedPackets {
		t.Errorf("Expected %d matched packets, but got %d",
			expectedPackets, *classifier.MatchedPackets)
	}
}
