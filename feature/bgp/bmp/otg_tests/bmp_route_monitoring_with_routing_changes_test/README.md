...
3) Interface Configuration: Configure IPv4 addresses on all involved interfaces on both the ATE and the DUT.
...
1)  Configure the ATE port-1 advertise a new prefix 203.0.113.0/24 to the DUT 
2)  Confirm that the route monitoring message contains the correct attributes: 
        • NLRI: 203.0.113.0/24 
        • Next-hop: ATE port-1 IP address 
...
1)  Withdraw Prefix: Configure ATE Port 1 to withdraw the prefix 203.0.113.0/24.
...
1)  Advertise with Initial Attributes: Configure ATE Port 1 to advertise the prefix 198.51.100.0/24 with the following attributes:

        • MED: 100 
        • Community: 65000:100 
        • Local preference: 100
2)  Validate Initial BMP Message: Verify that the received BMP Route Monitoring message contains the correct NLRI (198.51.100.0/24) and all the initially configured attributes.
3)  Re-advertise with Modified Attributes: Configure ATE Port 1 to re-advertise the same prefix (198.51.100.0/24) with modified attributes:
...
### BMP-1.2.4: Verify next-hop change monitoring 
...
### BMP-1.2.5: Verify policy filtering impact on monitoring

1)  Configure Import Policy: On the DUT, configure an import routing policy that applies to routes received from ATE Port 1 and modifies attributes for routes matching the prefix 192.0.2.0/24:
        • Set local preference to 300 
        • Add community 65000:300 
2)  Advertise Prefix: Configure ATE Port 1 to advertise the prefix 192.0.2.0/24.
3)  Validate Post-Policy Attributes: Confirm that the BMP Route Monitoring message received on the monitoring station for the prefix 192.0.2.0/24 reflects the post-policy attributes (Local Preference 300, Community 65000:300) as applied by the DUT's import policy.

### BMP-1.2.6: Verify multiple prefix updates monitoring 
...
### BMP-1.2.7: Verify route refresh impact on monitoring 
...
### BMP-1.2.8: Verify BGP flap monitoring 

1)  Advertise Prefix: Configure ATE Port 1 to advertise the prefix 203.0.113.0/24.
2)  Flap BGP Session: Simulate a BGP session flap by tearing down and immediately re-establishing the BGP peering between ATE Port 1 and DUT Port 1.
3)  Validate Withdrawal and Re-advertisement: Confirm that the BMP monitoring station receives a BMP Route Monitoring message indicating the withdrawal of 203.0.113.0/24 due to the session down event, followed by a BMP Route Monitoring message indicating the re-advertisement of the same prefix once the session is re-established. 
4)   Verify the sequence numbers and timestamps in the BMP messages to correlate the withdrawal and re-advertisement events with the BGP session flap.
