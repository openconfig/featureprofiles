...
func configureDUT(t *testing.T, dut *ondatra.DUTDevice) *gnmi.SetBatch {
	...
	gnmi.BatchReplace(batch, gnmi.OC().Interface(p3.Name()).Config(), dutP3.NewOCInterface(p3.Name(), dut))
	gnmi.BatchReplace(batch, gnmi.OC().Interface(p4.Name()).Config(), dutP4.NewOCInterface(p4.Name(), dut))
...
func configureDUTBGPNeighbors(t *testing.T, dut *ondatra.DUTDevice, batch *gnmi.SetBatch, bgp *oc.NetworkInstance_Protocol_Bgp) {
	...
	neighbors := []cfgplugins.BGPNeighborConfig{
		...
		},
		{
			AteAS:            ate3AS,
			PortName:         dutP3.Name,
			NeighborIPv4:     ateP3.IPv4,
			NeighborIPv6:     ateP3.IPv6,
			IsLag:            false,
			PolicyName:       ptrToString(policyName),
			MultiPathEnabled: false,
		},
...
func configureATE(t *testing.T, ate *ondatra.ATEDevice, bmpName string) gosnappi.Config {
	...
	atePort3 := ateConfig.Ports().Add().SetName(p3.ID())
	atePort4 := ateConfig.Ports().Add().SetName(p4.ID())
	...
	ateP3ConfigParams := ateConfigParams{
		atePort:      atePort3,
		atePortAttrs: ateP3,
		dutPortAttrs: dutP3,
		ateAS:        ate3AS,
	...
	ateP4ConfigParams := ateConfigParams{
		atePort:      atePort4,
		atePortAttrs: ateP4,
		dutPortAttrs: dutP4,
		bmpName:      bmpName,
	...
	configureBGPOnATEDevice(t, ateConfig, ateP2ConfigParams)
	configureBGPOnATEDevice(t, ateConfig, ateP3ConfigParams)
	// ATE Device 2 (BMP)
	configureBMPOnATEDevice(t, ateConfig, ateP4ConfigParams)
	return ateConfig
}
...
func verifyBMPPrePolicyRouteMonitoring(t *testing.T, ate *ondatra.ATEDevice, bmpName string) error {
	t.Helper()
...
func verifyBMPPrePolicyRouteMonitoringPerPrefix(t *testing.T, ate *ondatra.ATEDevice, bmpName string) error {
	t.Helper()
	...
	cases := []testCase{
		...
		},
		{
			...
			fn: func(t *testing.T) {
				...
				if err := verifyBMPPrePolicyRouteMonitoring(t, ate, bmpName); err != nil {
					t.Fatalf("BMP Pre-Policy Route Monitoring validation failed: %v", err)
				}
				if err := verifyBMPPrePolicyRouteMonitoringPerPrefix(t, ate, bmpName); err != nil {
					t.Fatalf("BMP Pre-Policy Route Monitoring validation failed: %v", err)
				}
			},
		},
