...
const (
	...
	v4LPTerminateStatement  = "lp-terminate-stmt-v4"
	v4ASPPolicy             = "asp-policy-v4"
	v4ASPStatement          = "asp-statement-v4"
	v4ASPTerminateStatement = "asp-terminate-stmt-v4"
	...
	v6LPTerminateStatement  = "lp-terminate-stmt-v6"
	v6ASPPolicy             = "asp-policy-v6"
	v6ASPStatement          = "asp-statement-v6"
	v6ASPTerminateStatement = "asp-terminate-stmt-v6"
...
func TestBGPNestedPolicies(t *testing.T) {
	...
	testCases := []testCase{
		{
			name:        "IPv4BGPNestedmportPolicy",
			desc:        "IPv4 BGP Nested import policy test",
		...
		},
		{
			...
			desc:        "IPv4 BGP Nested export policy test",
		...
		},
		{
			...
			desc:        "IPv6 BGP Nested import policy test",
		...
		},
		{
			...
			desc:        "IPv6 BGP Nested export policy test",
...
func configureImportRoutingPolicy(t *testing.T, dut *ondatra.DUTDevice) {
	...
	pdef1 := rp.GetOrCreatePolicyDefinition(v4LPPolicy)
	stmt1, err := pdef1.AppendNewStatement(v4LPStatement)
	if err != nil {
		t.Fatalf("AppendNewStatement(%s) failed: %v", v4LPStatement, err)
	}
	if !deviations.SkipSettingStatementForPolicy(dut) {
		t.Logf("Setting statement for policy")
		stmt1.GetOrCreateActions().SetPolicyResult(oc.RoutingPolicy_PolicyResultType_NEXT_STATEMENT)
	}
	stmt1.GetOrCreateActions().GetOrCreateBgpActions().SetSetLocalPref(localPref)

	stmt3, err := pdef1.AppendNewStatement(v4LPTerminateStatement)
	if err != nil {
		t.Fatalf("AppendNewStatement(%s) failed: %v", v4LPTerminateStatement, err)
	}
	if !deviations.SkipSettingStatementForPolicy(dut) {
	...
	pdef2 := rp.GetOrCreatePolicyDefinition(v4PrefixPolicy)
	stmt2, err := pdef2.AppendNewStatement(v4PrefixStatement)
	if err != nil {
...
func configureExportRoutingPolicy(t *testing.T, dut *ondatra.DUTDevice) {
	...
	t.Logf("Configuring export routing policy")
	...
	stmt3, err := pdef1.AppendNewStatement(v4ASPTerminateStatement)
	if err != nil {
		t.Fatalf("AppendNewStatement(%s) failed: %v", v4ASPTerminateStatement, err)
	}
	if !deviations.SkipSettingStatementForPolicy(dut) {
		stmt3.GetOrCreateActions().SetPolicyResult(oc.RoutingPolicy_PolicyResultType_ACCEPT_ROUTE)
	}
...
func validateExportRoutingPolicy(t *testing.T, dut *ondatra.DUTDevice, ate *ondatra.ATEDevice) {
	t.Logf("Validating Export Routing Policy, waiting for 120 seconds")
...
func configureImportRoutingPolicyV6(t *testing.T, dut *ondatra.DUTDevice) {
	...
	t.Logf("Configuring import routing policy")
	pdef1 := rp.GetOrCreatePolicyDefinition(v6LPPolicy)
	stmt1, err := pdef1.AppendNewStatement(v6LPStatement)
	if err != nil {
	...
	stmt3, err := pdef1.AppendNewStatement(v6LPTerminateStatement)
	if err != nil {
		t.Fatalf("AppendNewStatement(%s) failed: %v", v6LPTerminateStatement, err)
	}
	if !deviations.SkipSettingStatementForPolicy(dut) {
		stmt3.GetOrCreateActions().SetPolicyResult(oc.RoutingPolicy_PolicyResultType_ACCEPT_ROUTE)
	}

	pdef2 := rp.GetOrCreatePolicyDefinition(v6PrefixPolicy)
	stmt2, err := pdef2.AppendNewStatement(v6PrefixStatement)
	if err != nil {
	...
	if deviations.RoutePolicyUnderAFIUnsupported(dut) {
		//policy under peer group
		pathV4 := gnmi.OC().NetworkInstance(dni).Protocol(oc.PolicyTypes_INSTALL_PROTOCOL_TYPE_BGP, bgpName).Bgp().PeerGroup(peerGrpNamev4).ApplyPolicy()
		gnmi.BatchDelete(batch, pathV4.Config())
		...
		pathV4 := gnmi.OC().NetworkInstance(dni).Protocol(oc.PolicyTypes_INSTALL_PROTOCOL_TYPE_BGP, bgpName).Bgp().Neighbor(atePort1.IPv4).AfiSafi(oc.BgpTypes_AFI_SAFI_TYPE_IPV4_UNICAST).ApplyPolicy()
		gnmi.BatchDelete(batch, pathV4.Config())
...
func configureExportRoutingPolicyV6(t *testing.T, dut *ondatra.DUTDevice) {
	...
	stmt3, err := pdef1.AppendNewStatement(v6ASPTerminateStatement)
	if err != nil {
		t.Fatalf("AppendNewStatement(%s) failed: %v", v6ASPTerminateStatement, err)
	}
	if !deviations.SkipSettingStatementForPolicy(dut) {
		stmt3.GetOrCreateActions().SetPolicyResult(oc.RoutingPolicy_PolicyResultType_ACCEPT_ROUTE)
	}
