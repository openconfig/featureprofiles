...
func bgpCreateNbr(localAs, peerAs uint32, policy string) *telemetry.NetworkInstance_Protocol_Bgp {
	...
	d := &telemetry.Device{}
	ni1 := d.GetOrCreateNetworkInstance(defaultNIName)
...
func verifyBgpTelemetry(t *testing.T, dut *ondatra.DUTDevice) {
	...
	statePath := dut.Telemetry().NetworkInstance(defaultNIName).Protocol(telemetry.PolicyTypes_INSTALL_PROTOCOL_TYPE_BGP, "BGP").Bgp()
...
func TestEstablish(t *testing.T) {
	...
	dutConfPath := dut.Config().NetworkInstance(defaultNIName).Protocol(telemetry.PolicyTypes_INSTALL_PROTOCOL_TYPE_BGP, "BGP").Bgp()
...
func TestBGPPolicy(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			allFlows := configureATE(t, ate)
			dut.Config().NetworkInstance(defaultNIName).Protocol(telemetry.PolicyTypes_INSTALL_PROTOCOL_TYPE_BGP, "BGP").Bgp().Replace(t, bgp)
