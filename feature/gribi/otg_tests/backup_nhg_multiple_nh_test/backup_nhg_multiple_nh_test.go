...
func testIPv4BackUpSwitch(ctx context.Context, t *testing.T, args *testArgs) {
	...
	// create flow
	dstMac := gnmi.Get(t, args.ate, gnmi.OTG().Interface(atePort1.Name+".Eth").Ipv4Neighbor(dutPort1.IPv4).LinkLayerAddress().State())
...
func flapinterface(t *testing.T, dut *ondatra.DUTDevice, port string, action bool) {
	...
	gnmi.Update(t, dut, gnmi.OC().Interface(dutP.Name()).Config(), i)
}
...
func waitOTGARPEntry(t *testing.T) {
	...
	nbr := gnmi.OTG().Interface(atePort1.Name + ".Eth").Ipv4Neighbor(dutPort1.IPv4)
	gnmi.Watch(t, ate, nbr.LinkLayerAddress().State(),
		time.Minute, func(val *ygnmi.Value[string]) bool {
			return val.IsPresent()
		}).Await(t)
}
