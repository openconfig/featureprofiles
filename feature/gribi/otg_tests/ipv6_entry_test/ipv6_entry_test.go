...
const (
	...
	nh1IpAddr      = "2001:db8:2::22"
	nh2IpAddr      = "2001:db8:3::42"
)

const (
	// Destination prefix for DUT to ATE traffic (> 64 bits).
	dstPfx      = "2001:db8:100::/65"
	dstPfxMin   = "2001:db8:100::"
	dstPfxMax   = "2001:db8:100::ff"
	dstPfxCount = 256
)

const (
	// Destination prefixes for LPM discrimination test.
	dstIPv6Pfx64     = "2001:db8:200::/64"
	dstIPv6Pfx65More = "2001:db8:200::/65"
)
...
func staticARPWithMagicUniversalIP(t *testing.T, dut *ondatra.DUTDevice) {
	t.Helper()
	dummyIPCIDR1 := nh1IpAddr + "/128"
	dummyIPCIDR2 := nh2IpAddr + "/128"
	p2 := dut.Port(t, "port2")
	p3 := dut.Port(t, "port3")
...
func TestIPv6Entry(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			entries: []fluent.GRIBIEntry{
				fluent.NextHopEntry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
					WithIndex(nh1ID).WithIPAddress(atePort2.IPv6),
				fluent.NextHopEntry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
					WithIndex(nh2ID).WithIPAddress(atePort3.IPv6),
				fluent.NextHopGroupEntry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
					WithID(nhgID).
					AddNextHop(nh1ID, 1).
					AddNextHop(nh2ID, 1),
		...
		},
		{
			...
			wantGoodFlows: []string{"port3Flow"},
			wantBadFlows:  []string{"port2Flow"},
			wantOperationResults: []*client.OpResult{
				fluent.OperationResult().
					WithNextHopOperation(nh1ID).
					WithProgrammingResult(fluent.InstalledInFIB).
		...
		t.Run(fmt.Sprintf("Persistence=%s", persist), func(t *testing.T) {
			...
			for _, tc := range cases {
				t.Run(tc.desc, func(t *testing.T) {
					...
					defer func() {
						// Delete should reverse the order of entries, i.e. IPv6Entry must be removed
					...
					if tc.desc == "Multiple next-hops" {
						p2In := gnmi.Get(t, ate.OTG(), gnmi.OTG().Port(atePort2.Name).Counters().InFrames().State())
						p3In := gnmi.Get(t, ate.OTG(), gnmi.OTG().Port(atePort3.Name).Counters().InFrames().State())
						if p2In == 0 || p3In == 0 {
							t.Errorf("ECMP balancing failed: traffic not received on both ports: port2=%d, port3=%d", p2In, p3In)
						}
					}
				})
			}
	...
	// Subtest: LPM discrimination (/64 vs /65)
	t.Run("LPM_Discrimination_64_vs_65", func(t *testing.T) {
		c := fluent.NewClient()
		conn := c.Connection().
			WithStub(gribic).
			WithPersistence().
		...
		c.Modify().AddEntry(t,
			...
			fluent.NextHopGroupEntry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
				WithID(nhgID).AddNextHop(nh1ID, 1),
			fluent.IPv6Entry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
				WithPrefix(dstIPv6Pfx64).WithNextHopGroup(nhgID),
			fluent.NextHopEntry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
				WithIndex(nh2ID).WithIPAddress(atePort3.IPv6),
			fluent.NextHopGroupEntry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
				WithID(nhgID+1).AddNextHop(nh2ID, 1),
			fluent.IPv6Entry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
				WithPrefix(dstIPv6Pfx65More).WithNextHopGroup(nhgID+1),
		)
		if err := awaitTimeout(ctx, c, t, time.Minute); err != nil {
			t.Fatalf("Could not program LPM entries: %v", err)
		}

		otg := ate.OTG()
		ateTop := otg.GetConfig(t)
		ateTop.Flows().Clear().Items()
		// Flow 1: destined to more specific /65 -> should arrive on Port 3, not Port 2
		f1Good := createFlowWithPrefix(t, "LPM_MoreSpecific_Port3", ate, ateTop, "2001:db8:200::1", 256, &atePort3)
		f1Bad := createFlowWithPrefix(t, "LPM_MoreSpecific_Drop_Port2", ate, ateTop, "2001:db8:200::1", 256, &atePort2)
		// Flow 2: destined to /64 outside /65 -> should arrive on Port 2, not Port 3
		f2Good := createFlowWithPrefix(t, "LPM_Coarser_Port2", ate, ateTop, "2001:db8:200:0:8000::1", 256, &atePort2)
		f2Bad := createFlowWithPrefix(t, "LPM_Coarser_Drop_Port3", ate, ateTop, "2001:db8:200:0:8000::1", 256, &atePort3)
		otg.PushConfig(t, ateTop)
		otg.StartProtocols(t)

		validateTrafficFlows(t, ate, []string{f1Good, f2Good}, []string{f1Bad, f2Bad})
	})

	// Subtest: Boundary prefix length sweep (/65, /96, /126, /127, /128)
	t.Run("Boundary_Prefix_Length_Sweep", func(t *testing.T) {
		sweepPrefixes := []struct {
			name   string
			prefix string
			flowIP string
		}{
			{"Prefix_65", "2001:db8:101::/65", "2001:db8:101::1"},
			{"Prefix_96", "2001:db8:102::/96", "2001:db8:102::1"},
			{"Prefix_126", "2001:db8:103::/126", "2001:db8:103::1"},
			{"Prefix_127", "2001:db8:104::/127", "2001:db8:104::1"},
			{"Prefix_128", "2001:db8:105::1/128", "2001:db8:105::1"},
		}
			...
			t.Run(sp.name, func(t *testing.T) {
				...
				c.Modify().AddEntry(t,
					...
					fluent.NextHopGroupEntry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
						WithID(nhgID).AddNextHop(nh1ID, 1),
					fluent.IPv6Entry().WithNetworkInstance(deviations.DefaultNetworkInstance(dut)).
						WithPrefix(sp.prefix).WithNextHopGroup(nhgID),
				)
				if err := awaitTimeout(ctx, c, t, time.Minute); err != nil {
					t.Fatalf("Could not program prefix %s: %v", sp.prefix, err)
				}

				otg := ate.OTG()
				ateTop := otg.GetConfig(t)
				ateTop.Flows().Clear().Items()
				flowName := createFlowWithPrefix(t, "Flow_"+sp.name, ate, ateTop, sp.flowIP, 1, &atePort2)
				otg.PushConfig(t, ateTop)
				otg.StartProtocols(t)

				validateTrafficFlows(t, ate, []string{flowName}, nil)
			})
		}
	})
}
...
func createFlow(t *testing.T, name string, ate *ondatra.ATEDevice, ateTop gosnappi.Config, dsts ...*attrs.Attributes) string {
	return createFlowWithPrefix(t, name, ate, ateTop, dstPfxMin, dstPfxCount, dsts...)
}

func createFlowWithPrefix(t *testing.T, name string, ate *ondatra.ATEDevice, ateTop gosnappi.Config, startIP string, dstIPCount uint32, dsts ...*attrs.Attributes) string {
	...
	if dstIPCount > 1 {
		v6.Dst().Increment().SetStart(startIP).SetCount(dstIPCount)
	} else {
		v6.Dst().SetValue(startIP)
	}
	return name
}
...
func configStaticArp(p *ondatra.Port, ipv6addr string, macAddr string) *oc.Interface {
...
// setDUTInterfaceWithState sets the admin state on the dut interface
