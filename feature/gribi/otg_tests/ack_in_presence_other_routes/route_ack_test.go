...
func configStaticRoute(t *testing.T, dut *ondatra.DUTDevice, prefix string, nexthop string) *telemetry.NetworkInstance_Protocol_Static {
	...
	static := ni1.GetOrCreateProtocol(telemetry.PolicyTypes_INSTALL_PROTOCOL_TYPE_STATIC, *deviations.DefaultProtocolStaticName)
	...
	dut.Config().NetworkInstance(*deviations.DefaultNetworkInstance).Protocol(telemetry.PolicyTypes_INSTALL_PROTOCOL_TYPE_STATIC, *deviations.DefaultProtocolStaticName).Update(t, static)
	return sr
}
...
func TestRouteAck(t *testing.T) {
	...
	dut.Config().NetworkInstance(*deviations.DefaultNetworkInstance).Protocol(telemetry.PolicyTypes_INSTALL_PROTOCOL_TYPE_STATIC, *deviations.DefaultProtocolStaticName).Static(ateDstNetCIDR).Replace(t, dutConf)
	// Verify the entry for 203.0.113.0/24 is active through AFT Telemetry.
