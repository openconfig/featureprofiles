...
func TestRouteAck(t *testing.T) {
	...
	dut.Config().NetworkInstance(*deviations.DefaultNetworkInstance).Protocol(telemetry.PolicyTypes_INSTALL_PROTOCOL_TYPE_STATIC, *deviations.DefaultProtocolStaticName).Update(t, dutConf)
	// Verify the entry for 203.0.113.0/24 is active through AFT Telemetry.
