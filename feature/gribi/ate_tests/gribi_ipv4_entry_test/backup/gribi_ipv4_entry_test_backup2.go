...
func configureIPv4ViaClientAMultiNHs(t *testing.T, args *testArgs) {
	t.Logf("Adding an IPv4Entry for %s pointing to ATE port-2/Port-3 via clientA.", ateDstNetCIDR)
...
func configureIPv4ViaClientBInstalled(t *testing.T, args *testArgs) {
	t.Logf("Adding an IPv4Entry for %s pointing to ATE port-3 via clientB with election ID of 10.", ateDstNetCIDR)
...
//Single IPv4 Entry with NG with Multiple NHs
func configureIPv4ViaClientAMultipleNHs(t *testing.T, args *testArgs) {
	t.Logf("Adding an IPv4Entry for pointing to ATE port-2/port-3 via clientA with Multiple NHs")
...
//Single IPV4 Entry Multiple NHs.
func testSingleIPv4EntryMultipleNHs(ctx context.Context, t *testing.T, args *testArgs) {
	...
	//testTraffic(t, args.ate, args.top, srcEndPoint, dstEndPoint)
	testTrafficMultiNHs(t, args.ate, args.top, srcEndPoint, dstEndPoint)
...
func TestGribiIPv4EntryTE21(t *testing.T) {
	...
	t.Run(tt.name, func(t *testing.T) {
		...
		//Configure the gRIBI client clientB with election ID of 10
		clientB := fluent.NewClient()

		clientB.Connection().WithStub(gribic).WithInitialElectionID(10, 0).
			WithRedundancyMode(fluent.ElectedPrimaryClient)
