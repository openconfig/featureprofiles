...
func testTrafficBlackhole(t *testing.T, ate *ondatra.ATEDevice, top *ondatra.ATETopology, srcEndPoint, dstEndPoint *ondatra.Interface, recValue float32) {
	ethHeader := ondatra.NewEthernetHeader()
	ipv4Header := ondatra.NewIPv4Header()
	ipv4Header.DstAddressRange().
		//WithMin("198.51.100.0").
...
func testTrafficMultiNHs(t *testing.T, ate *ondatra.ATEDevice, top *ondatra.ATETopology, srcEndPoint, dstEndPoint1 *ondatra.Interface, dstEndPoint2 *ondatra.Interface, recValue float32) {
	ethHeader := ondatra.NewEthernetHeader()
	ipv4Header := ondatra.NewIPv4Header()
	ipv4Header.DstAddressRange().
		//WithMin("198.51.100.0").
	...
	flow := ate.Traffic().NewFlow("Flow").
		WithSrcEndpoints(srcEndPoint).
		WithDstEndpoints(dstEndPoint1, dstEndPoint2).
		WithHeaders(ethHeader, ipv4Header).WithFrameRateFPS(50)
	...
	if got := flowPath.LossPct().Get(t); got > recValue {
		t.Errorf("TestFail: LossPct for flow %s got %g, want 0", flow.Name(), got)
	} else {
		t.Logf("TestPass: LossPct for flow %s got %g, want 0", flow.Name(), got)
	}
...
func helperAddEntryDualRedundncy(ctx context.Context, t *testing.T, client *fluent.GRIBIClient) {
	t.Helper()
...
func testSingleIPv4EntrySingleNHG(ctx context.Context, t *testing.T, args *testArgs) {
	...
	testTraffic(t, args.ate, args.top, srcEndPoint, dstEndPoint, 0.0)
}
...
func testSingleIPv4EntryMultipleNHs(ctx context.Context, t *testing.T, args *testArgs) {
	...
	helperAddEntryDualRedundncy(ctx, t, args.clientA)
	...
	testTrafficMultiNHs(t, args.ate, args.top, srcEndPoint, dstEndPoint1, dstEndPoint2, 0.0)

}

//Single IPV4 Entry Invalid NH. Client A
func testSingleIPv4EntryInvalidNH(ctx context.Context, t *testing.T, args *testArgs) {
	// Configure IPv4 route for 1.0.0.0/8 pointing to invalid NH 192.0.2.60
	helperAddEntry(ctx, t, args.clientA, "192.0.2.60", nhIndexA, nhgIndexA)
	...
	srcEndPoint := args.top.Interfaces()[atePort1.Name]
	dstEndPoint := args.top.Interfaces()[atePort2.Name]
	testTrafficBlackhole(t, args.ate, args.top, srcEndPoint, dstEndPoint, 100.0)

	//Update nexthop to down interface
	//Bringdown interface dut - ate port3
	helperAddEntry(ctx, t, args.clientA, "192.0.2.10", nhIndexA, nhgIndexA)

	ipv4Path = args.dut.Telemetry().NetworkInstance("default").Afts().Ipv4Entry(ateDstNetCIDR)
	if got, want := ipv4Path.Prefix().Get(t), ateDstNetCIDR; got != want {
		t.Logf("1.0.0.0/8 with invalid nexthop 192.0.2.60 is not in active state")
	}
...
func tTestSingleIPv4EntrySingleNH(t *testing.T) {
	...
	t.Run(tt.name, func(t *testing.T) {
		...
		//## Fix Add election id check here.
...
func TestSingleIPv4EntryInvalidNHs(t *testing.T) {
	dut := ondatra.DUT(t, "dut")
