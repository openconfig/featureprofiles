...
// configureInterfaceDUT configures a single DUT interface
func configureInterfaceDUT(t *testing.T, dut *ondatra.DUTDevice, dutPort *ondatra.Port, d *oc.Root, desc string) {
	t.Helper()
	iName := dutPort.Name()
...
func generateSubIntfPair(t *testing.T, dut *ondatra.DUTDevice, dutPort *ondatra.Port, _ *ondatra.ATEDevice, atePort *ondatra.Port, top *ondatra.ATETopology, d *oc.Root) []string {
	nextHops := []string{}
...
func configureSubinterfaceDUT(t *testing.T, d *oc.Root, dut *ondatra.DUTDevice, dutPort *ondatra.Port, index uint32, vlanID uint16, dutIPv4 string, vrf string) {
	t.Helper()
	...
	g := gnmi.OC()
	gnmi.Update(t, dut, g.Interface(dutPort.Name()).Config(), i)

	if vrf != *deviations.DefaultNetworkInstance || *deviations.ExplicitInterfaceInDefaultVRF {
		fptest.AssignToNetworkInstance(t, dut, d.GetOrCreateInterface(dutPort.Name()).GetName(), vrf, index)
	}
}
...
func TestScaling(t *testing.T) {
	...
	configureSubinterfaceDUT(t, d, dut, dp1, 0, 0, dutPort1.IPv4, vrf1)
