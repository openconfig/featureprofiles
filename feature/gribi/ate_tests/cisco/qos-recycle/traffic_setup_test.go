...
func (a *testArgs) allFlowsQos() []*ondatra.Flow {
	...
	dscps := []uint8{1, 9, 17, 25, 33, 41, 49}
	flows := []*ondatra.Flow{}
	flownames := []string{"tc1", "tc2", "tc3", "tc4", "tc5", "tc6", "tc7"}
	for i, dscp := range dscps {
		flow := a.createFlowQos(flownames[i], srcEndPoint, dstEndPoint, innerdstPfxMin_bgp, dscp, innerdstPfxCount_bgp)
		flows = append(flows, flow)
	}

	return flows
}
func (a *testArgs) allFlowsQosScale() []*ondatra.Flow {
	srcEndPoint := a.top.Interfaces()[atePort1.Name]
	dstEndPoint := []ondatra.Endpoint{}
	for intf, intf_data := range a.top.Interfaces() {
		if intf != "atePort1" {
	...
	flownames := []string{"tc1", "tc2", "tc3", "tc4", "tc5", "tc6", "tc7"}
	for i, dscp := range dscps {
		flow := a.createFlowQosScale(flownames[i], srcEndPoint, dstEndPoint, innerdstPfxMin_bgp, dscp, innerdstPfxCount_bgp)
		flows = append(flows, flow)
	}

	return flows
}
func (a *testArgs) createFlowQos(name string, srcEndPoint *ondatra.Interface, dstEndPoint []ondatra.Endpoint, innerdstPfxMin string, Dscp uint8, innerdstPfxCount uint32) *ondatra.Flow {
	hdr := ondatra.NewIPv4Header()
	hdr.WithSrcAddress(dutPort1.IPv4).WithDSCP(Dscp).DstAddressRange().WithMin(dstPfxMin).WithCount(uint32(*ciscoFlags.GRIBIScale)).WithStep("0.0.0.1")

	innerIpv4Header := ondatra.NewIPv4Header()
	//innerIpv4Header.WithSrcAddress(innersrcPfx).WithDSCP(Dscp)
	...
	flow := a.ate.Traffic().NewFlow(name).
		...
		WithHeaders(ondatra.NewEthernetHeader(), hdr, innerIpv4Header).WithFrameRatePct(10).WithFrameSize(300)

	return flow
}
func (a *testArgs) createFlowQosScale(name string, srcEndPoint *ondatra.Interface, dstEndPoint []ondatra.Endpoint, innerdstPfxMin string, Dscp uint8, innerdstPfxCount uint32) *ondatra.Flow {
	hdr := ondatra.NewIPv4Header()
	hdr.WithSrcAddress(dutPort1.IPv4).WithDSCP(Dscp).DstAddressRange().WithMin(dstPfxMin).WithCount(uint32(100)).WithStep("0.0.0.1")
...
func (a *testArgs) ValidateQosStats(t *testing.T, d_port []string) {

	inpackets := []uint64{}
	flowstats := gnmi.GetAll(t, a.ate, gnmi.OC().FlowAny().Counters().State())
	...
	queueNames := []string{}
	for _, EgressInterface := range d_port {
		...
		t.Run(fmt.Sprintf("Get Interface Telemetry %s", EgressInterface), func(t *testing.T) {
			gote := gnmi.Get(t, a.dut, interfaceTelemetryEgrPath)
	...
	for name := range queuestats {
		...
		if name == "tc7" {
			...
			t.Log("Comparing qos stats for non tc7 queues")
