...
func testRemAddHWWithGNMIReplaceAndPBRwithOC(ctx context.Context, t *testing.T, args *testArgs) {
	...
	policy := telemetry.NetworkInstance_PolicyForwarding{}
	path := args.dut.Config().NetworkInstance("default").PolicyForwarding()
	config.GNMICommitReplaceWithOC(context.Background(), t, args.dut, baseConfigWithPBRWithoutHWModule, path, &policy)
	...
	_, basePolicy := getBasePBROCConfig(t, args)
	config.GNMICommitReplaceWithOC(context.Background(), t, args.dut, baseConfigWithoutPBRWithHWModule, path, basePolicy)
	config.Reload(context.Background(), t, args.dut, "", "", 6*time.Minute)
	args.clientA.StartWithNoCache(t)
	args.clientA.BecomeLeader(t)
	configureBaseDoubleRecusionVip1Entry(ctx, t, args)
