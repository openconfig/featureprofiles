...
func (tc *testCase) verifyAFTConsistency(t *testing.T, aftSession1, aftSession2 *aftcache.AFTStreamSession, desc string) (*aftcache.AFTData, error) {
	t.Helper()
	aft1, err := aftSession1.ToAFT(t, tc.dut)
	if err != nil {
		return nil, fmt.Errorf("error getting AFT from session 1 for %s verification: %v", desc, err)
	}
	aft2, err := aftSession2.ToAFT(t, tc.dut)
	if err != nil {
		return nil, fmt.Errorf("error getting AFT from session 2 for %s verification: %v", desc, err)
	}
	sortSlices := cmpopts.SortSlices(func(a, b uint64) bool { return a < b })
	if diff := cmp.Diff(aft1, aft2, sortSlices); diff != "" {
		return nil, fmt.Errorf("AFTs from two sessions are not consistent for %s verification: %s", desc, diff)
	}
	return aft1, nil
}
...
func checkMemoryUsage(t *testing.T, dut *ondatra.DUTDevice, history *usageHistory, memBefore uint64, desc string, procName ...string) uint64 {
	...
	if increase > maxMemoryIncreasePercent {
		t.Errorf("Memory usage for process %s increased by %.2f%%, which is more than the %.2f%% threshold.", pName, increase, maxMemoryIncreasePercent)
	} else {
		t.Logf("Memory usage change for process %s is %.2f%%, which is within the %.2f%% threshold.", pName, changePct, maxMemoryIncreasePercent)
	}
	if history != nil {
...
func checkCPUUsage(t *testing.T, dut *ondatra.DUTDevice, history *usageHistory, cpuBefore uint64, desc string, cpuComponent string, procName ...string) uint64 {
	...
	switch dut.Vendor() {
	case ondatra.ARISTA:
		t.Logf("Checking CPU usage %s.", desc)
	...
	changePct := math.Max(0, increase)
	if increase > maxCPUIncreasePercent {
		t.Errorf("CPU usage for process %s increased by %.2f%%, which is more than the %.2f%% threshold.", pName, increase, maxCPUIncreasePercent)
	} else {
		t.Logf("CPU usage change for process %s is %.2f%%, which is within the %.2f%% threshold.", pName, changePct, maxCPUIncreasePercent)
	}
	if history != nil {
		history.add(usageRecord{
			time:      time.Now(),
...
func TestSlowCollector(t *testing.T) {
	...
	defer func() {
		if tc.gnmiClient1 != nil {
			if closer, ok := tc.gnmiClient1.(io.Closer); ok {
				if err := closer.Close(); err != nil {
					t.Logf("Error closing gNMI client 1: %v .", err)
				}
			}
		}
	}()
	...
	go func() {
		defer wg.Done()
		aftSession1.ListenUntil(sessionCtx1, t, aftConvergenceTime, initialStoppingCondition)
	}()
	go func() {
		defer wg.Done()
	...
	aft, err := tc.verifyAFTConsistency(t, aftSession1, aftSession2, "Initial AFT verification")
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Log("Starting slow collector test, by canceling both collector context...")
	cancelSession1()
	...
	// Recreate client 1 with no latency.
	baseClient1, err := dut.RawAPIs().BindingDUT().DialGNMI(t.Context())
	if err != nil {
		t.Fatalf("Failed to dial GNMI for collector 1: %v", err)
	}
	tc.gnmiClient1 = baseClient1

	// Recreate client 2 with 250ms latency injected via metadata.
	baseClient2, err := dut.RawAPIs().BindingDUT().DialGNMI(t.Context())
	if err != nil {
		t.Fatalf("Failed to dial GNMI for collector 2: %v", err)
	}
	tc.gnmiClient2 = baseClient2

	sessionCtx1, cancelSession1 = context.WithCancel(t.Context())
	defer cancelSession1()
	measuringClient1 := &latencyMeasuringGNMIClient{
		GNMIClient: baseClient1,
		t:          t,
		colName:    "Collector1",
	}
	aftSession1 = aftcache.NewAFTStreamSession(sessionCtx1, t, measuringClient1, dut)

	sessionCtx2, cancelSession2 = context.WithCancel(t.Context())
	sessionCtx2 = metadata.AppendToOutgoingContext(sessionCtx2, "latency", "250ms")
	defer cancelSession2()
	measuringClient2 := &latencyMeasuringGNMIClient{
		GNMIClient:      baseClient2,
		t:               t,
		colName:         "Collector2",
		expectedLatency: 250 * time.Millisecond,
	}
	aftSession2 = aftcache.NewAFTStreamSession(sessionCtx2, t, measuringClient2, dut)

	t.Log("Waiting for restarted collectors to converge...")
	stoppingCondition := aftcache.InitialSyncStoppingCondition(t, dut, wantPrefixes, wantIPv4NHs, wantIPv6NHs)
	convergeAndWait := func() {
		var wg sync.WaitGroup
		wg.Add(2)
		go func() {
			defer wg.Done()
	...
	waitAndVerify := func(monitoringDesc, consistencyDesc string, nhCount int, verifyDeleted bool) {
		...
		if dut.Vendor() == ondatra.ARISTA {
			...
			go func() {
				...
				for {
					select {
					case <-ticker.C:
						checkMemoryUsage(t, dut, &history, aristaMemBaseline, "during "+monitoringDesc)
						for comp, cpuBase := range aristaCPUBaselines {
							checkCPUUsage(t, dut, &history, cpuBase, "during "+monitoringDesc, comp)
						}
					case <-memDone:
						return
					}
				...
				for {
					...
					case <-ticker.C:
						for procName, memBase := range ciscoMemBaseline {
							checkMemoryUsage(t, dut, &history, memBase, "during "+monitoringDesc, procName)
						}
						for procName, cpuBase := range ciscoCpuBaseline {
							checkCPUUsage(t, dut, &history, cpuBase, "during "+monitoringDesc, "", procName)
						}
					case <-memDone:
						return
					}
		...
		if dut.Vendor() == ondatra.ARISTA {
			aristaMemBaseline = checkMemoryUsage(t, dut, &history, aristaMemBaseline, "after "+monitoringDesc)
			for comp, cpuBase := range aristaCPUBaselines {
				aristaCPUBaselines[comp] = checkCPUUsage(t, dut, &history, cpuBase, "after "+monitoringDesc, comp)
			}
		} else if dut.Vendor() == ondatra.CISCO {
			for procName, memBase := range ciscoMemBaseline {
				ciscoMemBaseline[procName] = checkMemoryUsage(t, dut, &history, memBase, "after "+monitoringDesc, procName)
			}
			for procName, cpuBase := range ciscoCpuBaseline {
				ciscoCpuBaseline[procName] = checkCPUUsage(t, dut, &history, cpuBase, "after "+monitoringDesc, "", procName)
			}
		}
		...
		aftn, err := tc.verifyAFTConsistency(t, aftSession1, aftSession2, consistencyDesc)
		if err != nil {
			t.Fatal(err)
		}
		...
		if verifyDeleted {
			...
			if err := tc.verifyPrefixesDeleted(t, aftn, startingBGPRouteIPv4, int(routeCount(dut, IPv4))); err != nil {
				t.Errorf("Failed to verify IPv4 BGP prefixes removed: %v", err)
			}
			if err := tc.verifyPrefixesDeleted(t, aftn, startingBGPRouteIPv6, int(routeCount(dut, IPv6))); err != nil {
				t.Errorf("Failed to verify IPv6 BGP prefixes removed: %v", err)
			}
			if _, err := tc.verifyAFTConsistency(t, aftSession1, aftSession2, "No BGP prefixes after deletion"); err != nil {
				t.Fatalf("failed to get AFT Cache after deletion: %v", err)
			}
		} else {
			if err := tc.verifyPrefixes(t, aftn, startingBGPRouteIPv4, int(routeCount(dut, IPv4)), nhCount); err != nil {
				t.Errorf("Failed to verify IPv4 BGP prefixes after %s: %v", monitoringDesc, err)
			}
			if err := tc.verifyPrefixes(t, aftn, startingBGPRouteIPv6, int(routeCount(dut, IPv6)), nhCount); err != nil {
	...
	waitAndVerify("collector's convergence", "after-latency-injection", 2, false)
...
// latencyMeasuringGNMIClient wraps a GNMIClient to measure Recv() latency.
type latencyMeasuringGNMIClient struct {
	gnmipb.GNMIClient
	t               *testing.T
	colName         string
...
// Subscribe overrides the base client's Subscribe to return a wrapped stream.
func (c *latencyMeasuringGNMIClient) Subscribe(ctx context.Context, opts ...grpc.CallOption) (gnmipb.GNMI_SubscribeClient, error) {
	stream, err := c.GNMIClient.Subscribe(ctx, opts...)
	if err != nil {
		return nil, err
	}
	return &latencyMeasuringSubscribeClient{
		GNMI_SubscribeClient: stream,
		t:                    c.t,
		colName:              c.colName,
		expectedLatency:      c.expectedLatency,
	}, nil
}

// latencyMeasuringSubscribeClient wraps a GNMI_SubscribeClient to measure Recv() latency.
type latencyMeasuringSubscribeClient struct {
	gnmipb.GNMI_SubscribeClient
	t               *testing.T
	colName         string
	expectedLatency time.Duration
	mu              sync.Mutex
...
// Recv measures the time taken by the underlying Recv() call.
func (c *latencyMeasuringSubscribeClient) Recv() (*gnmipb.SubscribeResponse, error) {
	start := time.Now()
	resp, err := c.GNMI_SubscribeClient.Recv()
	...
	// Log the first few messages to confirm latency.
	if currentCount <= 20 {
		c.t.Logf("[%s] Recv #%d took %v", c.colName, currentCount, elapsed)
		if c.expectedLatency > 0 && elapsed < c.expectedLatency {
			//c.t.Errorf("[%s] Recv #%d took %v, which is less than expected latency %v. Proxy injection might be failing.", c.colName, currentCount, elapsed, c.expectedLatency)
		}
	}
	return resp, err
}
