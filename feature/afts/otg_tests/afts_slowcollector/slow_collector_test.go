...
func (tc *testCase) restartCollectorsWithLatency(t *testing.T, b *usageBaselines, wantPrefixes map[string]bool, history *usageHistory) (context.Context, context.CancelFunc, *aftcache.AFTStreamSession, context.Context, context.CancelFunc, *aftcache.AFTStreamSession) {
	...
	/*sessionCtx1 = metadata.AppendToOutgoingContext(sessionCtx1, "latency", "1ms")*/
	measuringClient1 := &latencyMeasuringGNMIClient{
		GNMIClient: tc.gnmiClient1,
		t:          t,
		colName:    "Collector1",
	}
	aftSession1 := aftcache.NewAFTStreamSession(sessionCtx1, t, measuringClient1, tc.dut)

	// Session 2 (250ms)
	sessionCtx2, cancelSession2 := context.WithCancel(t.Context())
	sessionCtx2 = metadata.AppendToOutgoingContext(sessionCtx2, "latency", "10ms")
	measuringClient2 := &latencyMeasuringGNMIClient{
		GNMIClient:      tc.gnmiClient2,
		t:               t,
		colName:         "Collector2",
		expectedLatency: 250 * time.Millisecond,
	}
	aftSession2 := aftcache.NewAFTStreamSession(sessionCtx2, t, measuringClient2, tc.dut)
...
func (tc *testCase) verifyConvergence(
	...
	if verifyDeleted {
		...
		tc.verifyAFTConsistency(t, aftSession1, aftSession2, "No BGP prefixes after deletion")
	} else {
...
func TestSlowCollector(t *testing.T) {
	...
	tc.verifyConvergence(t, sessionCtx1, aftSession1, sessionCtx2, aftSession2, stoppingCondition, startTime, "route withdrawal convergence", "post-route-withdrawal-port2", 1, false)
	...
	tc.verifyConvergence(t, sessionCtx1, aftSession1, sessionCtx2, aftSession2, stoppingCondition, startTime, "complete route withdrawal convergence", "post-route-withdrawal-all", 0, true)
	...
	tc.verifyConvergence(t, sessionCtx1, aftSession1, sessionCtx2, aftSession2, stoppingCondition, startTime, "re-advertise convergence", "post-readvertise-port1", 1, false)
	...
	tc.verifyConvergence(t, sessionCtx1, aftSession1, sessionCtx2, aftSession2, stoppingCondition, startTime, "re-advertise convergence", "post-readvertise-all", 2, false)
...
func (c *latencyMeasuringSubscribeClient) Recv() (*gnmipb.SubscribeResponse, error) {
	...
	if currentCount <= 20 {
		...
		if c.expectedLatency > 0 && elapsed < c.expectedLatency {
			c.t.Logf("[%s] Recv #%d took %v, which is less than expected latency %v. Proxy injection might be failing.", c.colName, currentCount, elapsed, c.expectedLatency)
		}
	}
	return resp, err
}
