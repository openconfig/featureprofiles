...
import (
	...
	"github.com/google/gnmi/metadata"
...
func checkCPUUsage(t *testing.T, dut *ondatra.DUTDevice, history *usageHistory, cpuBefore uint64, desc string, cpuComponent string, procName ...string) uint64 {
	...
	changePct := math.Max(0, increase)
	if cpuAfter > cpuThreshold {
		t.Errorf("CPU usage for process %s increased by %.2f%%, which is more than the %.f%% threshold.", pName, increase, cpuThreshold)
	} else {
		t.Logf("CPU usage change for process %s is %.2f%%, which is within the threshold.", pName, changePct)
	}
	if history != nil {
		history.add(usageRecord{
			time:      time.Now(),
...
func (tc *testCase) verifyConvergence(
	...
	// Verify ISIS prefixes.
	if verifyDeleted {
		t.Log("Verifying ISIS prefixes are removed...")
		if err := tc.verifyPrefixesDeleted(t, aftn, startingISISRouteIPv4, isisRouteCount); err != nil {
			t.Errorf("Failed to verify IPv4 ISIS prefixes removed: %v", err)
		}
		if err := tc.verifyPrefixesDeleted(t, aftn, startingISISRouteIPv6, isisRouteCount); err != nil {
			t.Errorf("Failed to verify IPv6 ISIS prefixes removed: %v", err)
		}
	} else {
		// Verify ISIS prefixes are present in AFT.
...
func TestSlowCollector(t *testing.T) {
	...
	for p := range netutil.GenCIDRs(t, startingISISRouteIPv4, isisRouteCount) {
		wantAllPrefixes[p] = true
	}
	for p := range netutil.GenCIDRs(t, startingISISRouteIPv6, isisRouteCount) {
		wantAllPrefixes[p] = true
	}
	startTime = time.Now()
