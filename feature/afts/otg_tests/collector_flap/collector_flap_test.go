...
const (
	...
	bgpRouteCountIPv4LowScale = 1000 
...
func TestCollectorFlap(t *testing.T) {
	...
	aft := tc.verifyAFTConsistency(t, aftSession1, aftSession2, "initial")

	t.Log("AFT is consistent. Verifying prefixes...")
	if err := tc.verifyPrefixes(t, aft, startingBGPRouteIPv4, int(getRouteCount(dut, IPv4)), 2); err != nil {
		t.Errorf("failed to verify initial IPv4 BGP prefixes: %v", err)
	}
	if err := tc.verifyPrefixes(t, aft, startingBGPRouteIPv6, int(getRouteCount(dut, IPv6)), 2); err != nil {
	...
	t.Log("ISIS verification completed Starting collector restart test")

	var memBeforeRestart uint64
	if dut.Vendor() != ondatra.CISCO && dut.Vendor() != ondatra.JUNIPER && dut.Vendor() != ondatra.NOKIA {
		t.Log("Getting memory usage before restarting collector 2")
		memBeforeRestart = gnmi.Get(t, dut, gnmi.OC().System().Memory().Used().State())
	} else {
		t.Logf("Skipping memory usage check for %v device.", dut.Vendor())
	}

	t.Log("Stopping collector 2 by canceling its context...")
	...
	t.Log("AFT is consistent. Verifying prefixes...")
	if err := tc.verifyPrefixes(t, aftn, startingBGPRouteIPv4, int(getRouteCount(dut, IPv4)), 2); err != nil {
		t.Errorf("failed to verify IPv4 BGP prefixes after collector 2 flap: %v", err)
	}
	if err := tc.verifyPrefixes(t, aftn, startingBGPRouteIPv6, int(getRouteCount(dut, IPv6)), 2); err != nil {
		t.Errorf("failed to verify IPv6 BGP prefixes after collector 2 flap: %v", err)
	}

	switch dut.Vendor() {
	case ondatra.JUNIPER, ondatra.NOKIA, ondatra.CISCO:
		// TODO: Add memory usage check for Cisco, Juniper and Nokia device.
		t.Logf("Skipping memory usage check for %v device.", dut.Vendor())
	default:
		t.Log("Getting memory usage after restarting collector 2")
		memAfterRestart := gnmi.Get(t, dut, gnmi.OC().System().Memory().Used().State())
		if memBeforeRestart > 0 {
			increase := (float64(memAfterRestart) - float64(memBeforeRestart)) / float64(memBeforeRestart) * 100
			if increase > 2.0 {
				t.Errorf("Memory usage increased by %.2f%%, which is more than the 2%% threshold.", increase)
			} else {
				t.Logf("Memory usage change is %.2f%%, which is within the 2%% thr+eshold.", increase)
			}
		}
	}
}
