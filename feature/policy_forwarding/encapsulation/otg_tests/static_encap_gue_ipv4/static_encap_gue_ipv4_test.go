...
func validatePackets(t *testing.T, filename string, protocolType string, tos string, ttl uint8) {
	...
	for packet := range packetSource.Packets() {
		...
		if !ok || udp == nil {
			...
			var innerPacketTOS, innerttl uint8
			dscp := ""
			...
			case "ipv4":
				...
				dscp = fmt.Sprintf("0x%X", innerPacketTOS)
				innerttl = ip.TTL
			case "ipv6":
				...
				dscp = fmt.Sprintf("0x%X", innerPacketTOS)
				innerttl = ip.HopLimit
			}

			if ttl != 0 {
				if innerttl == ttl {
					t.Logf("TTL matched: expected ttl %d, got ttl %d", ttl, innerttl)
				} else {
					t.Errorf("TTL mismatch: expected ttl %d, got ttl %d", ttl, innerttl)
				}
			}
			if dscp != "" {
				if dscp == tos {
					t.Logf("TOS matched: expected TOS %v, got TOS %v", tos, dscp)
				} else {
					t.Errorf("TOS mismatch: expected TOS %v, got TOS %v", tos, dscp)
				}
			}
	...
	for packet := range packetSource.Packets() {
		...
		if ipv6Layer != nil {
			...
			if icmpv6Layer != nil {
				...
				if icmpPacket.TypeCode != layers.ICMPv6TypeTimeExceeded && icmpPacket.TypeCode != layers.ICMPv6CodeNoRouteToDst {
					t.Logf("PASS: received ICMPv6 type-11, code-0")
				} else {
					t.Errorf("FAIL: did not received ICMPv6 type-11, code-0, Got Code: %v , type: %v", icmpPacket.TypeCode.Code(), icmpPacket.TypeCode.Type())
				}
			}
		}
	}
...
func TestGUEEncap(t *testing.T) {
	...
	t.Run("RT-3.53.1: IPv4 GUE Encap without explicit ToS/TTL", func(t *testing.T) {
		...
		for _, p := range capturePorts {
			...
			validatePackets(t, capture, "ipv4", "0x80", 9)
		}
	...
	t.Run("RT-3.53.2: IPv6 traffic GUE encapsulation without explicit ToS/TTL configuration on tunnel", func(t *testing.T) {
		...
		for _, p := range capturePorts {
			capture := processCapture(t, ate.OTG(), p)
			validatePackets(t, capture, "ipv6", "0x80", 9)
		}
	...
	t.Run("RT-3.53.3: IPv4 GUE Encap with explicit ToS", func(t *testing.T) {
		...
		for _, p := range capturePorts {
			capture := processCapture(t, ate.OTG(), p)
			validatePackets(t, capture, "ipv4", "0x60", 0)
		}
	})
	...
	t.Run("RT-3.53.4: IPv6 GUE Encap with explicit ToS", func(t *testing.T) {
		...
		// TODO: explicit tos is not seen in inner header, once its supported, will add code
		// for _, p := range capturePorts {
		// 	capture := processCapture(t, ate.OTG(), p)
		// 	validatePackets(t, capture, "ipv6", "0x60", 9)
		// }
	})
	...
	t.Run("RT-3.53.5: IPv4 GUE Encap with explicit TTL", func(t *testing.T) {
		...
		// for _, p := range capturePorts {
		// 	capture := processCapture(t, ate.OTG(), p)
		// 	validatePackets(t, capture, "ipv4", "0x80", 19)
		// }
	})
	...
	t.Run("RT-3.53.6: IPv6 GUE Encap with explicit TTL", func(t *testing.T) {
		...
		// for _, p := range capturePorts {
		// 	capture := processCapture(t, ate.OTG(), p)
		// 	validatePackets(t, capture, "ipv6", "0x80", 19)
		// }
	})
	...
	t.Run("RT-3.53.7: IPv4 traffic GUE encapsulation with explicit ToS and TTL configuration on tunnel", func(t *testing.T) {
		...
		// // TODO: once TTL support added, will add the code
		for _, p := range capturePorts {
			capture := processCapture(t, ate.OTG(), p)
			validatePackets(t, capture, "ipv4", "0x60", 0)
		}
	...
	t.Run("RT-3.53.8: IPv6 traffic GUE encapsulation with explicit ToS and TTL configuration on tunnel", func(t *testing.T) {
		...
		// TODO: once support TOS / TTL in ipv6 added, will add the code
		// for _, p := range capturePorts {
		// 	capture := processCapture(t, ate.OTG(), p)
		// 	validatePackets(t, capture, "ipv6", "0x60", 19)
		// }
	})
	...
	t.Run("RT-3.53.9: IPv4 traffic GUE encapsulation to a single 5-tuple tunnel", func(t *testing.T) {
		...
		t.Log("Verify traffic packets")
		verifySinglePathTraffic(t, ate, otgConfig, "IPv4-GUE-Single-Traffic")

		// TODO: once support TOS / TTL in ipv6 added, will add the code
		// capture := processCapture(t, ate.OTG(), activeport[0])
		// validatePackets(t, capture, "ipv4", "0x60", 9)
	...
	t.Run("RT-3.53.10: IPv6 traffic GUE encapsulation to a single 5-tuple tunnel", func(t *testing.T) {
		...
		t.Log("Verify traffic packets")
		verifySinglePathTraffic(t, ate, otgConfig, "IPv6-GUE-Single-Traffic")
