...
func gueDecapInnerIpv4Traffic(t *testing.T, dut *ondatra.DUTDevice, ate *ondatra.ATEDevice, topo gosnappi.Config, ateUdpPort int, dutUdpPort int, destIp string, trafficValidation bool, verifyCounters bool, countersMatch bool) {
	...
	configureIPv4Traffic(t, dut, ate, topo, trafficID, ateUdpPort, destIp)
...
func gueDecapInnerIpv6Traffic(t *testing.T, dut *ondatra.DUTDevice, ate *ondatra.ATEDevice, topo gosnappi.Config, ateUdpPort int, dutUdpPort int, destIp string, trafficValidation bool, verifyCounters bool, countersMatch bool) {
	trafficID := fmt.Sprintf("Gue-Decap-Flow1-%v", ateUdpPort)
	configureIPv6Traffic(t, dut, ate, topo, trafficID, ateUdpPort, destIp)
	...
	if deviations.DecapsulateGueOCUnsupported(dut) {
		...
		case ondatra.ARISTA:
			cliConfig = fmt.Sprintf(`
							...
							`, guePort, ipType, ipType, guePort, tunIp, dp1.Name())
...
func configureIPv4Traffic(t *testing.T, dut *ondatra.DUTDevice, ate *ondatra.ATEDevice, topo gosnappi.Config, trafficID string,
...
func configureIPv6Traffic(t *testing.T, dut *ondatra.DUTDevice, ate *ondatra.ATEDevice, topo gosnappi.Config, trafficID string,
	guePort int, destIp string) {
	t.Logf("Configure Traffic from ATE with flowname %s", trafficID)
	topo.Flows().Clear()
	macAddress := gnmi.Get(t, dut, gnmi.OC().Interface(dut.Port(t, "port1").Name()).Ethernet().MacAddress().State())
