...
func TestTrustBundleCert(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.concatenate {
				t.Logf("%s:STATUS:%s:Building combined trust bundle for rotation.", time.Now().String(), tc.desc)
				if err := buildCombinedTrustBundle(tc.trustBundleFile, []string{"01", "02"}, "rsa"); err != nil {
					t.Fatalf("%s:STATUS:%s:failed to build combined trust bundle: %v", time.Now().String(), tc.desc, err)
				}
			}
