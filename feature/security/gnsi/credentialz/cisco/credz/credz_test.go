...
func createSSHClient(host string, port int, user string, privateKeyPath string) error {
	...
	log.Infof("Successfully connected using %v private key!", privateKeyPath)
	...
	_, file_name := filepath.Split(privateKeyPath)
	if strings.Contains(output, authentication_type[file_name]) {
		log.Infof("Authentication type of user(%s) is %s as expected", user, authentication_type[file_name])
	} else {
		err_msg := fmt.Sprintf("Authentication type of user(%s) is not %s", user, authentication_type[file_name])
		log.Error(err_msg)
...
func TestPubKeyAuthentication(t *testing.T) {
	...
	users := []*authz.User{}
	users = append(users, &authz.User{Name: account_name})
	...
	for _, pvt_file := range pvt_key_names {
		err = createSSHClient(target_ip, target_port, credentials_data.Account, pvt_file)
		if err != nil {
