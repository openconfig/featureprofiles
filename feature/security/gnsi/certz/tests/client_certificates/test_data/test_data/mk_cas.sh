  ...
  for k in $(seq 1 ${d}); do
    ...
    for t in ${TYPES[@]}; do
      ...
      for g in a b; do
        case ${t} in
          rsa)
            openssl genrsa -out ca-${d}/ca-${OFFSET}-${t}-${g}-key.pem ${RSAKEYLEN}
            ;;
          ecdsa)
            openssl ecparam -name ${CURVE} \
              -out ca-${d}/ca-${OFFSET}-${t}-${g}-key.pem -genkey
            ;;
        esac
        # Create a cert with the fresh key.
        openssl req -new -x509 -nodes -days ${LIFETIME} \
          -key ca-${d}/ca-${OFFSET}-${t}-${g}-key.pem \
          -out ca-${d}/ca-${OFFSET}-${t}-${g}-cert.pem \
          -subj "/CN=CA ${OFFSET}/C=AQ/ST=NZ/L=NZ/O=OpenConfigFeatureProfiles"
      done
    done
  done
...
for d in ${DIRS[@]}; do
  for t in ${TYPES[@]}; do
    ...
    for g in a b; do
      cat ca-${d}/ca-*-${t}-${g}-cert.pem > ca-${d}/trust_bundle_${d}_${t}_${g}.pem
    done
  done
done
