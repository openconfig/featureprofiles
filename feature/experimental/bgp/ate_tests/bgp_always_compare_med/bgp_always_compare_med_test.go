...
func setMED(t *testing.T, dut *ondatra.DUTDevice, d *oc.Root) {
	...
	if deviations.RoutePolicyUnderAFIUnsupported(dut) {
		gnmi.Replace(t, dut, dutPolicyConfPath2.ApplyPolicy().ImportPolicy().Config(), []string{setMEDPolicy100, rplAllowPolicy})
		gnmi.Replace(t, dut, dutPolicyConfPath3.ApplyPolicy().ImportPolicy().Config(), []string{setMEDPolicy50, rplAllowPolicy})
	} else {
		gnmi.Replace(t, dut, dutPolicyConfPath2.AfiSafi(oc.BgpTypes_AFI_SAFI_TYPE_IPV4_UNICAST).
			ApplyPolicy().ImportPolicy().Config(), []string{setMEDPolicy100, rplAllowPolicy})
		gnmi.Replace(t, dut, dutPolicyConfPath3.AfiSafi(oc.BgpTypes_AFI_SAFI_TYPE_IPV4_UNICAST).
			ApplyPolicy().ImportPolicy().Config(), []string{setMEDPolicy50, rplAllowPolicy})
	}
}
