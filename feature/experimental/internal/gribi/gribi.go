...
// Flush performs a gribi flush of routes from within a given network instance
func (c *Client) Flush(t testing.TB, instance string) {
	fr, err := c.fluentC.Flush().
		WithElectionID(c.InitialElectionIDLow, c.InitialElectionIDHigh).
		WithNetworkInstance(instance).
		Send()
	switch {
	case err != nil:
		t.Fatalf("Got unexpected error from flush operation: %v", err)
	case fr == nil:
		t.Fatalf("Got nil response from flush operation: %v", fr)
	}
}
