...
func TestFtiTunnels(t *testing.T) {
	...
	wantLoss := true
	loadbalance := []bool{true, true, true, true, true, true, true, true}
	t.Run("Verify load balance and traffic drops with IPv4 and IPv6 flow via 8 tunnel", func(t *testing.T) {
		...
		VerifyUnderlayOverlayLoadbalanceTest(t, p, dut1, dut2, rt, dut1Intf1, dut1Intf2, dut1Intf3, dut1Intf4, dut2Intf1, dut2Intf2, dut2Intf3, dut2Intf4, 8, wantLoss, loadbalance)
	})
	// Disable and enable  the numbers of Tunnel interface being used
	// Disable 5 tunnel interfaces
	// All 5 at once
	...
	t.Run("Telemetry", func(t *testing.T) {
		disableAndEnableTunnelInterface(t, dut1, enable)
	})

	// Static route using the disable interface should become invalid and should not be used in forwarding traffic
	// Traffic loss is expcted here for those disabled interface
	// Disabling and enabling the number of tunnel interfaces and related static route shouldn’t case traffic drops
	wantLoss = true
	loadbalance = []bool{false, false, false, false, false, true, true, true}
	t.Run("Verify load balance and traffic drops with IPv4 and IPv6 flow via 3 tunnel", func(t *testing.T) {
		t.Log("Verify load balance and traffic drops with IPv4 and IPv6 flow via 3 tunnel")
		VerifyUnderlayOverlayLoadbalanceTest(t, p, dut1, dut2, rt, dut1Intf1, dut1Intf2, dut1Intf3, dut1Intf4, dut2Intf1, dut2Intf2, dut2Intf3, dut2Intf4, 3, wantLoss, loadbalance)
	})
	...
	wantLoss = true
	loadbalance = []bool{true, true, true, true, true, true, true, true}
	t.Run("Verify load balance and traffic drops with IPv4 and IPv6 flow via agin 8 tunnel", func(t *testing.T) {
		t.Log("Verify load balance and traffic drops with IPv4 and IPv6 flow via agin 8 tunnel")
		VerifyUnderlayOverlayLoadbalanceTest(t, p, dut1, dut2, rt, dut1Intf1, dut1Intf2, dut1Intf3, dut1Intf4, dut2Intf1, dut2Intf2, dut2Intf3, dut2Intf4, 8, wantLoss, loadbalance)
	})
}
...
func configureOtg(t *testing.T, otg *otg.OTG, p *parameters) gosnappi.Config {
	...
	iDut1Ipv6.SetAddress(p.rtIntf1Ipv6Add).SetGateway(p.r0Intf1Ipv6Add).SetPrefix(uint32int32(p.ipv6Mask))
...
func VerifyTraffic(t *testing.T, ate *ondatra.ATEDevice, flowName string, wantLoss bool) {
	...
	if wantLoss {
		if lossPct > uint64(tolerancePct) {
			t.Errorf("Traffic Loss Pct for Flow: %s but got %v, want 0", flowName, lossPct)
		} else {
			t.Logf("No Traffic Loss Test Passed!!")
		}
	} else {
		if lossPct < 100-uint64(tolerancePct) {
			t.Errorf("Traffic is expected to fail but flow :%s  got %v, want 100%% failure", flowName, lossPct)
		} else {
...
func VerifyUnderlayOverlayLoadbalanceTest(t *testing.T, p *parameters, dut1 *ondatra.DUTDevice, dut2 *ondatra.DUTDevice, rt *ondatra.ATEDevice, dut1Intf1 *ondatra.Port, dut1Intf2 *ondatra.Port, dut1Intf3 *ondatra.Port, dut1Intf4 *ondatra.Port, dut2Intf1 *ondatra.Port, dut2Intf2 *ondatra.Port, dut2Intf3 *ondatra.Port, dut2Intf4 *ondatra.Port, ftiIntfCount int64, wantLoss bool, loadbalance []bool) {
	...
	for i, flowName := range flows {
		...
		VerifyTraffic(t, rt, flowName, wantLoss)
	}
	...
	// Incoming traffic flow should be equally distributed for Encapsulation(ECMP)
	t.Logf("Verify Underlay loadbalancing 2 interface - Incoming traffic flow should be equally distributed for Encapsulation(ECMP) ")
	for key, _ := range finalInfStats {
		VerifyLoadbalance(t, 4, p.trafficRate, p.trafficDuration, 2, int64(initialInfStats[key]), int64(finalInfStats[key]), wantLoss)
	}
	...
	if deviations.TunnelStatePathUnsupported(dut1) {
		...
		keys := []string{}
		for m := range dut1FinalFitStats {
			keys = append(keys, m)
		}
		sort.Strings(keys)
		for j, key := range keys {
			VerifyLoadbalance(t, 4, p.trafficRate, p.trafficDuration, ftiIntfCount, int64(dut1InitialFitStats[key]), int64(dut1FinalFitStats[key]), loadbalance[j])
		}
	...
	if deviations.TunnelStatePathUnsupported(dut2) {
		...
		keys := []string{}
		for n := range dut2FinalFitStats {
			keys = append(keys, n)
		}
		sort.Strings(keys)
		for k, key := range keys {
			VerifyLoadbalance(t, 4, p.trafficRate, p.trafficDuration, ftiIntfCount, int64(dut2InitialFitStats[key]), int64(dut2FinalFitStats[key]), loadbalance[k])
		}
	}
}

func disableAndEnableTunnelInterface(t *testing.T, dut *ondatra.DUTDevice, state bool) {
