...
var (
	...
	tracerouteipv6InLayers layers.EthernetType = 0x86DD
...
func packetTracerouteRequestGet(isIPv4 bool, ttl uint8) []byte {
	...
	pktICMP6 := &layers.ICMPv4{
		TypeCode: layers.ICMPv6TypeTimeExceeded,
		Checksum: checksum,
	}
	...
	if isIPv4 {
		...
		gopacket.SerializeLayers(buf, opts,
			pktEthipv6, pktIpv6, pktICMP6, gopacket.Payload(payload),
		)
