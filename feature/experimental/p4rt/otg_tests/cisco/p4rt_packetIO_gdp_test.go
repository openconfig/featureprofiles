...
func (gdp *GDPPacketIO) GetTrafficFlow(t *testing.T, ate *ondatra.ATEDevice, frameSize uint32, frameRate uint64) []gosnappi.Flow {
	f := gosnappi.NewFlow()
	f.SetName("GDP")
	eth1 := f.Packet().Add().Ethernet()
	eth1.Src().SetValue(*gdp.SrcMAC)
	eth1.Dst().SetValue(*gdp.DstMAC)
	eth1.EtherType().SetValue(*gdp.EthernetType)
...
func (ttl *GDPPacketIO) IsIPv6(t *testing.T) bool {
	return false
}
