...
type TrafficFlow interface {
	...
	GetTTL() uint32
	SetTTL(ttl uint32)
	GetTrafficFlow(t *testing.T) gosnappi.Flow
}
...
type PacketIO interface {
	...
	GetTrafficFlow(t *testing.T, ate *ondatra.ATEDevice, frameSize uint32, frameRate uint64) []gosnappi.Flow
		...
		if packet != nil {
			...
			if wantPacket.udp {
				if wantPacket.udp {
					...
					if udpLayer := packet.Layer(layers.LayerTypeUDP); udpLayer == nil {
						t.Fatalf("UDP header in Packet is not matching wanted packet.")
					}
			...
			if (wantPacket.DstIPv4 != nil || wantPacket.DstIPv6 != nil) && !wantPacket.udp {
				...
				if *wantPacket.SrcIPv4 != srcIP && *wantPacket.SrcIPv6 != srcIP && *wantPacket.DstIPv4 != dstIP && *wantPacket.DstIPv6 != dstIP {
					...
					t.Fatalf("IP header in Packet is not matching wanted packet.")
				}
			}
...
func testP4RTTraffic(t *testing.T, ate *ondatra.ATEDevice, top gosnappi.Config, flows []gosnappi.Flow, duration int, ipv6 bool) {
	top.Flows().Clear()
...
func testEntryProgrammingPacketIn(ctx context.Context, t *testing.T, args *testArgs) {
	...
	if err := programmTableEntry(ctx, t, client, args.packetIO, false); err != nil {
		t.Fatalf("There is error when inserting the match entry")
	}
...
func testEntryProgrammingPacketInAndNoReply(ctx context.Context, t *testing.T, args *testArgs) {
	...
	if count_1-count_0 > 20 {
		t.Fatalf("Unexpected replies are sent from router!")
	}
}
...
func testEntryProgrammingPacketInWithNewIP(ctx context.Context, t *testing.T, args *testArgs, ipAddress string, isIPv6 bool) {
	...
	testP4RTTraffic(t, args.ate, args.top, flows, 10, args.packetIO.IsIPv6(t))
...
func testEntryProgrammingPacketInDowngradePrimaryController(ctx context.Context, t *testing.T, args *testArgs) {
	...
	if err := client.StreamChannelSendMsg(&streamName, &p4_v1.StreamMessageRequest{
		...
		t.Fatalf("There is error when sending arbitration message, %s", err)
	}
	if _, _, arbErr := client.StreamChannelGetArbitrationResp(&streamName, 1); arbErr != nil {
		t.Fatalf("There is error when downgrading the client, %s", arbErr)
	}
	...
	t.Logf("Captured packets: %v", len(packets))
	if len(packets) != 0 {
		t.Fatalf("There are unexpected packets received.")
	}
}
...
func testEntryProgrammingPacketInWithInnerTTL(ctx context.Context, t *testing.T, args *testArgs) {
	...
	flows := args.packetIO.GetTrafficFlow(t, args.ate, 300, 2)
	for _, flow := range flows {
		headers := flow.Packet().Items()
		for _, header := range headers {
			if header.HasIpv4() {
				header.Ipv4().Src().SetValue("1.1.1.1")
				header.Ipv4().Dst().SetValue("2.2.2.2")
				header.Ipv4().TimeToLive().SetValue(1)
			}
			if header.HasIpv6() {
				header.Ipv6().Src().SetValue("1::1")
				header.Ipv6().Dst().SetValue("2::2")
				header.Ipv6().HopLimit().SetValue(1)
			}
		}
	}
...
func testEntryProgrammingPacketInWithMalformedPacket(ctx context.Context, t *testing.T, args *testArgs) {
	...
	for _, flow := range flows {
		headers := flow.Packet().Items()
		for _, header := range headers {
			if header.HasEthernet() {
				if strings.Contains(flow.Name(), "IPv4") {
					header.Ethernet().EtherType().SetValue(0x0800)
				}
				if strings.Contains(flow.Name(), "IPv6") {
					header.Ethernet().EtherType().SetValue(0x8600)
				}
			}
		}
	}
...
func testEntryProgrammingPacketInWithUDP(ctx context.Context, t *testing.T, args *testArgs) {
	...
	for _, flow := range flows {
		...
		udpHeader.SrcPort().SetValue(11111)
		udpHeader.DstPort().SetValue(33433)
	}
...
func testEntryProgrammingPacketInWithFlowLabel(ctx context.Context, t *testing.T, args *testArgs) {
	...
	for _, flow := range flows {
		...
		for _, header := range headers {
			if header.HasIpv6() {
				header.Ipv6().FlowLabel().SetValue(11111)
...
func testEntryProgrammingPacketInScaleRate(ctx context.Context, t *testing.T, args *testArgs) {
	...
	// Send Packet
	// srcEndPoint := args.top.Interfaces()[atePort1.Name]
	testP4RTTraffic(t, args.ate, args.top, args.packetIO.GetTrafficFlow(t, args.ate, 300, 1000), 5, args.packetIO.IsIPv6(t))
...
func testPacketOut(ctx context.Context, t *testing.T, args *testArgs) {
	...
	atePort := sortPorts(args.ate.Ports())[0]
	aicp0 := gnmi.Get(t, args.ate.OTG(), gnmi.OTG().Port(atePort.ID()).State())
	ate_counter0 := aicp0.GetCounters().GetInFrames()
	...
	aicp1 := gnmi.Get(t, args.ate.OTG(), gnmi.OTG().Port(atePort.ID()).State())
	ate_counter1 := aicp1.GetCounters().GetInFrames()
...
func testPacketOutEgressWithChangePortId(ctx context.Context, t *testing.T, args *testArgs) {
	...
	if counter_1-counter_0 < uint64(float64(packet_count)*0.90) {
		t.Fatalf("There are not enought packets recived.")
	}
}
