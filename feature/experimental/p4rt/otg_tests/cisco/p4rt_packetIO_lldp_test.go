...
func (lldp *LLDPPacketIO) GetTrafficFlow(t *testing.T, ate *ondatra.ATEDevice, frameSize uint32, frameRate uint64) []gosnappi.Flow {
	f := gosnappi.NewFlow()
	f.SetName("LLDP")
	eth1 := f.Packet().Add().Ethernet()
	eth1.Src().SetValue(*lldp.SrcMAC)
	eth1.Dst().SetValue(*lldp.DstMAC)
	eth1.EtherType().SetValue(*lldp.EthernetType)
	f.Size().SetFixed(uint32(frameSize))
	f.Rate().SetPps(uint64(frameRate))
	return []gosnappi.Flow{f}
}
...
func (ttl *LLDPPacketIO) IsIPv6(t *testing.T) bool {
	return false
}
