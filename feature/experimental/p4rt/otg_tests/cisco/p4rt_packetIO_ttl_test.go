...
func (ttl *TTLPacketIO) GetTrafficFlow(t *testing.T, ate *ondatra.ATEDevice, frameSize uint32, frameRate uint64) []gosnappi.Flow {
	if ttl.IPv4 {
		flow := gosnappi.NewFlow()
		flow.SetName("TTL-IPv4")
		ethHeader := flow.Packet().Add().Ethernet()
		ethHeader.Src().SetValue(*ttl.SrcMAC)
		ethHeader.Dst().SetValue(*ttl.DstMAC)
		ipHeader := flow.Packet().Add().Ipv4()
		ipHeader.Src().SetValue(*ttl.SrcIPv4)
		ipHeader.Dst().SetValue(*ttl.DstIPv4)
		ipHeader.TimeToLive().SetValue(uint32(*ttl.TTL))
	...
	flow := gosnappi.NewFlow()
	flow.SetName("TTL-IPv6")
	ethHeader := flow.Packet().Add().Ethernet()
	ethHeader.Src().SetValue(*ttl.SrcMAC)
	ethHeader.Dst().SetValue(*ttl.DstMAC)
	ipv6Header := flow.Packet().Add().Ipv6()
	ipv6Header.Src().SetValue(*ttl.SrcIPv6)
	ipv6Header.Dst().SetValue(*ttl.DstIPv6)
	ipv6Header.HopLimit().SetValue(uint32(*ttl.TTL))
	flow.Size().SetFixed(uint32(frameSize))
	flow.Rate().SetPps(uint64(frameRate))
	return []gosnappi.Flow{flow}
}
...
func (ttl *TTLPacketIO) IsIPv6(t *testing.T) bool {
	return ttl.IPv6
}
