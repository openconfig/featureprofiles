...
func getTTLParameter(t *testing.T, dstMac string, ipv4, ipv6, ttlTwo bool) PacketIO {
	return &TTLPacketIO{
		PacketIOPacket: PacketIOPacket{
			SrcMAC:  ygot.String("00:01:00:01:00:01"),
			DstMAC:  ygot.String(dstMac),
...
func TestP4RTPacketIO(t *testing.T) {
	...
	if *ciscoFlags.TTLTests {
		...
		if *ciscoFlags.TTL1v4 {
			ttlTestcases["IPv4 TTL1 Only"] = func() { args.packetIO = getTTLParameter(t, dstMac, true, false, false) }
		}
		if *ciscoFlags.TTL1n2v4 {
			ttlTestcases["IPv4 TTL1 and TTL2"] = func() { args.packetIO = getTTLParameter(t, dstMac, true, false, true) }
		}
		if *ciscoFlags.TTL1v6 {
			ttlTestcases["IPv6 TTL1 Only"] = func() { args.packetIO = getTTLParameter(t, dstMac, false, true, false) }
		}
		if *ciscoFlags.TTL1n2v6 {
			ttlTestcases["IPv6 TTL1 and TTL2"] = func() { args.packetIO = getTTLParameter(t, dstMac, false, true, true) }
		}
		if *ciscoFlags.TTL1v4n6 {
			ttlTestcases["IPv4 TTL1 and IPv6 TTL1"] = func() { args.packetIO = getTTLParameter(t, dstMac, true, true, false) }
		}
		if *ciscoFlags.TTL1n2v4n6 {
			ttlTestcases["IPv4 TTL1 and TTL2 and IPv6 TTL1 and TTL2"] =
				func() {
					args.packetIO = getTTLParameter(t, dstMac, true, true, true)
				}
		}
