...
func saveOCJSON(val *oc.Root, path string, containerFilter[]string, leafFilter[]string)  {
	...
	mapVal, err := ygot.ConstructIETFJSON(val,&marshalCFG); if err != nil {
	...
	jsonConfig, err = json.MarshalIndent(mapVal,"","	"); if err != nil {
		panic(fmt.Sprintf("Cannot marshal oc struct in to json : %v", err))
	}
	err = os.WriteFile(path, jsonConfig, 0644); if err!= nil {
...
func main() {
	...
	_,err =batchRep.Set(context.Background(), ygnmiCli); if err!=nil {
