...
def RunGoTest(self: FireXTask, ws, testsuite_id, test_log_directory_path, xunit_results_filepath,
    ...
    try:
        ...
        if suite: 
            ...
            core_files_only = suite.attrib['failures'] == '0'
            if collect_debug_files or core_files_only:
...
def CollectDebugFiles(self, ws, internal_fp_repo_dir, reserved_testbed, test_log_directory_path, timestamp, core_files_only, xunit_results_filepath) -> str:
...
# noinspection PyPep8Naming
@app.task(bind=True)
def AppendCoreFileCheckResult(self, test_log_directory_path, xunit_results_filepath):
    try:
