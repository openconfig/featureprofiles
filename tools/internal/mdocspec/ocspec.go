...
// ParseCanonicalOCs extracts all the Canonical OCs from a featureprofiles README.
// If such a section is not found in the README, `ErrCanonicalOCNotFound` will be
// returned.
//
// Expected markdown format:
//
...
// ignored.
func ParseCanonicalOCs(source []byte) ([]ygot.GoStruct, error) {
	var buf bytes.Buffer
	md := goldmark.New(
	...
	if len(MDJSONSpecs.canonicalOCs) == 0 {
		return nil, ErrCanonicalOCNotFound
	}
	var ocs []ygot.GoStruct
	for _, oc := range MDJSONSpecs.canonicalOCs {
...
func getCanonicalOC(source []byte) (ygot.GoStruct, error) {
	d := &fpoc.Device{}
	if err := fpoc.Unmarshal(source, d); err != nil {
		return nil, err
	}
	return d, nil
}
