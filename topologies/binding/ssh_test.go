...
// sshClientFixture configures a local SSH server and client connected over
...
var sshServerSigner ssh.Signer

func init() {
	var err error
	sshServerSigner, err = ssh.ParsePrivateKey(sshServerKeyPEM())
	if err != nil {
		panic(err)
	}
}

func (f *sshClientFixture) start(t testing.TB) error {
	...
	serverConfig.AddHostKey(sshServerSigner)
...
func (f *sshClientFixture) handleServerNewChannel(ch <-chan ssh.NewChannel) {
...
func (f *sshClientFixture) handleServerChannel(c ssh.Channel, reqs <-chan *ssh.Request) {
	var shell, pty bool
	r := bufio.NewReader(c)
...
//   - stderr: exec stderr
func (f *sshClientFixture) handleExec(c ssh.Channel, req *ssh.Request) {
...
func TestSSH(t *testing.T) {
	...
	if err := f.start(t); err != nil {
		t.Fatalf("Could not start sshClientFixture: %v", err)
	}
	t.Log("Test is ready.")
	defer f.sshClient.Close()

	res, err := f.sshClient.RunCommand(context.Background(), "xyzzy")
	if err != nil {
