...
// DHCPRelayConfigParams holds the parameters for configuring DHCP relay on a DUT interface.
type DHCPRelayConfigParams struct {
	Interface         string
	IPv4HelperAddress string
	IPv6HelperAddress string
}

// DhcpRelayConfig returns the CLI config string for DHCP relay on a given subinterface.
func DhcpRelayConfig(t *testing.T, dut *ondatra.DUTDevice, sb *gnmi.SetBatch, cfgParams DHCPRelayConfigParams) *gnmi.SetBatch {
	if deviations.DhcpRelayOcUnsupported(dut) {
		switch dut.Vendor() {
		case ondatra.ARISTA:
			dhcpRelayConfig := new(strings.Builder)
			fmt.Fprintf(dhcpRelayConfig, `
		interface %[1]s
		ip dhcp relay all-subnets
		ip helper-address %[2]s
		ipv6 dhcp relay destination %[3]s`, cfgParams.Interface, cfgParams.IPv4HelperAddress, cfgParams.IPv6HelperAddress)
