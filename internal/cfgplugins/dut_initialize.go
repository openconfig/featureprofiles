...
const (
	...
	FeaturePolicyForwardingTTLHandling
	FeatureQOSCounters
...
`
   ...
   `
	...
	aristaTcamProfilePolicyForwardingTTLHandling = `
   hardware tcam
   profile ngpr
      system-rule overriding-action redirect
      !
      ...
      feature traffic-policy port ipv6
         sequence 25
         key size limit 160
         key field dst-ipv6-label dst-port hop-limit icmp-type-code ipv6-length ipv6-next-header ipv6-traffic-class l4-dst-port-label l4-src-port-label src-ipv6-label tcp-control
         action count drop redirect set-dscp set-tc set-unshared-policer
         packet ipv6 forwarding bridged
         packet ipv6 forwarding routed
      !
...
var (
	aristaTcamProfileMap = map[FeatureType]string{
		...
		FeaturePolicyForwardingTTLHandling: aristaTcamProfilePolicyForwardingTTLHandling,
