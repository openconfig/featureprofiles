...
func (c *Client) GetElectionID(ctx context.Context, t testing.TB) (low, high uint64) {
	mu.Lock()
	defer mu.Unlock()
	_, ok := electionID[c.DUT]
	if !ok {
		electionID[c.DUT] = gribi.Uint128{Low: 0,
			High: 0,
		}
	}
	return electionID[c.DUT].Low, electionID[c.DUT].High
}
